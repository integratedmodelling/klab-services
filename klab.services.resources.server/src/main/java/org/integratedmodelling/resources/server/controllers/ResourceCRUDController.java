...
public class ResourceCRUDController {
    ...
    /**
     * GET endpoint that returns uncontextualized resource data to the authorized user. If the resource is
     * public, the endpoint works w/o authentication.
     *
     * @param urn
     * @param principal
     */
    ...
    /**
     * POST endpoint that takes geometry and (possibly) value URLs for dependencies from the
     * {@link org.integratedmodelling.klab.api.digitaltwin.DigitalTwin} and returns the contextualized data as
     * a flow.
     */
    @Secured(Role.USER)
    ...
    /**
     * PUT endpoint that creates a new resource, returning the new URN (takes hints for it but may not follow
     * them). Uses multipart data. Note that permissions are handled separately; at creation, the resource is
     * only available to the creating identity
     */
    @Secured(Role.USER)
