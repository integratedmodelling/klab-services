...
public interface Contextualizable extends KlabStatement {
    ...
    public enum Trigger {
        ...
        /**
         * Definition, i.e. initialization. Only legal with perdurants. Bound to 'on definition' [set to | do
         * ] For instantiators, 'self' is the context of the new instances.
         */
        DEFINITION,
        ...
        /**
         * Instantiation: before resolution of EACH new instance from an instantiator. Not accepted within
         * contextualizers. 'self' is the new instance, 'context' their context on which the instantiator was
         * called.
         */
        INSTANTIATION,

        /**
         * The default trigger for resources that provide the main content for models (including instantiating
         * resources) unless the model is for an occurrent, in which case the resource gets the TRANSITION
         * trigger.
         */
        RESOLUTION,
    ...
    /**
     * Target ID: if null, the main observable of the model, otherwise another observable which must be
     * defined. This is a syntactic property and can be accessed outside of a contextualization scope.
     *
    ...
    /**
     * The target observable for this computation, correspondent to the target ID. Accessible only during
     * contextualization. Null if the target is the main observable in the correspondent actuator. Otherwise
     * the computation affects other artifacts, as in the case of internal dependencies due to indirect
     ...
     * this artifact through this mediator".
     * <p>
     * This may be merged with getTarget() at some point as the use cases for it are similar.
     *
    ...
    /**
     * Each computation may use a different language. Null means the default supported expression language.
     *
     ...
     * getUrn(), getClassification(), getAccordingTo(), getLookupTable() and getExpression() will return a
     * non-null value.
     *
     * @return any literal
     */
     ...
     * getClassification(), getAccordingTo(), getLookupTable() and getExpression() will return a non-null
     * value.
     *
    ...
    /**
     * An implicit classification built by matching values of an annotation property to subclasses of the
     * observable. Only one among getLiteral(), getServiceCall(), getUrn(), getClassification(),
    ...
    /**
     * A URN specifying a remote computation. Only one among getLiteral(), getServiceCall(), getUrn() and
     ...
     * form (scalar or not) by the runtime environment.
     *
    ...
    /**
     * Any parameters set for the computation, e.g. in the case of a function call or a URN with optional
     * values.
     *
    ...
    /**
     * In interactive mode, resources may expose parameters for users to check and modify before execution.
     * Implementation-dependent services will extract descriptors and set values.
     *
    ...
    /**
     * This computation may be linked to a condition, which is another computation producing a boolean. This
     * is always empty if this resource is itself a condition.
     *
    ...
    /**
     * The computation may consist in a mediation of a quantity represented by the first element in the
     * returned tuple, which must be converted into a value represented by the second.
     * <p>
    ...
    /**
     * Only meaningful if this computable is a condition computing a (scalar or distributed) boolean, this
     * specifies whether this condition was given with negative ('unless' instead of 'if') semantics.
     *
    ...
    /**
     * True if this computation is a mediation, expected to output a transformation of the artifact passed to
     * it, to be used in its place to match a specific observation semantics.
     *
    ...
    /**
     * If true, this defines an accessory variable rather than a dependency. The targetId is the name of the
     * variable.
     *
     * @return
     */
     ...
     * contextualize() if there's no temporal change for example.
     * <p>
