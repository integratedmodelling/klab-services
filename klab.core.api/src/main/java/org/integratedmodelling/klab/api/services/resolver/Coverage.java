 ...
 * <p>
 * A coverage with no extents and coverage proportion == 1.0 is a "universal" coverage that can be
 * merged into another with no effect. Anything covering everything should have that coverage.
 * 
 * In addition to the percent covered, a coverage may also include scale constraints that restrict
 * the type of scales an asset can be used in. This includes specific extents, resolutions, or
 * ranges thereof for each of the extents, or the statement that a specific extent must be present
 * without further specification.
 * <p>
 * A {@code Coverage} redefines the {@link Scale#merge(TopologicallyComparable, LogicalConnector)}
 ...
 * <p>
 * Due to the collapsing of extents and the reinterpretation of the meaning of merging, it is
...
public interface Coverage extends Scale {
    ...
    /**
     * True if the coverage has no extents and covers them all. This can be merged into anything to
     * not change it, and anything merged into it returns the merged coverage.
     * 
     * @return
     */
    boolean isUniversal();
    ...
    /**
     * This creates a coverage from a geometry, with {@link #getCoverage()} == 1. Because the
     * geometry does not have to be fully specified, the resulting coverage may be unusable as a
     * scale but can have constraints that can be used to match other scales for compatibility.
     * 
     * @param geometry
     * @return
     */
     ...
     * coverage will have the constraints of being isomorphic to the scale in terms of
     * representation, as the scale must be fully specified and worldview-bound.
     * 
     * @param scale
     * @param coverage
     * @return
    ...
    /**
     * The universal coverage can be bi-directionally merged with any other in any mode to return
     * the merged coverage with gain == 1 if a valid coverage is merged with a universal one.
     * 
     * @return
     */
    ...
    /**
     * Empty coverages will make any intersected coverage empty (gain == -1 unless the merged
     * coverage was empty) and won't change a coverage when unioned with it, so gain will be 0.
     * 
     * @return
     */
