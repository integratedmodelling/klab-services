 ...
 * <p>
 * The default behavior when black/whitelisting a model is to only set the
 * constraint to resolve its own observable. All other constraints are global
 * unless otherwise specified.
 * <p>
 * Any resolution constraints are found as a collection in a {@link Scope}'s
 * metadata using the key {@link #RESOLUTION_CONSTRAINTS_METADATA_KEY}.
 * 
 * @author Ferd
 *
 */
public interface ResolutionConstraint {
	...
	/**
	 * Return true if the model is for a different observable or it's for the passed
	 * observable but part of a whitelist, is not blacklisted, comes from an
	 * accepted namespace, and uses accepted resources.
	 * 
	 * @param model
	 * @return
	 */
	 ...
	 * {@link #accepts(IModel, IObservable)} when the model passed to it contains
	 * resources.
	 * 
	 * @param model
	 * @return
