...
/**
 * A service prototype that defines a function API and potentially holds the class/method that
 * implements it. In k.LAB prototypes apply to commands and language functions so far, both of which
 * can return one of these. Prototypes are automatically created by the {@link KlabFunction}
...
public interface ServiceInfo extends Serializable {
  ...
  enum FunctionType {
    ...
    ANNOTATION,
    /**
     * Parameters associated to Java method based on type among the unnamed arguments. If named args
     * are present, those are passed as Parameters to the first argument matching Parameters or Map.
     */
    FREEFORM
  }

  // tags for arguments or methods, representing roles inferred by annotations
  enum Tag {
    INPUT,
    OUTPUT
  }
  ...
  interface Argument extends Serializable {
    ...
    /**
     * Arguments with the status of parameter are modifiable by the user in interactive workflows.
     * The use of the modifiable assignment '?=' should be conditioned to the corresponding argument
     * being a parameter.
     *
     * @return
     */
  ...
  /**
   * Observations needed by a contextualizer, matched to the context by the name in the dependency
   * observables.
   *
   * @return imports, or an empty list.
   */
  List<Argument> listInputs();

  /**
   * Additional observations contextualized by the function, created before the call only if they
   * are referenced in the dataflow. Matched by name in the output observables.
   *
   * @return exports, or an empty list.
   */
  List<Argument> listOutputs();
   ...
   * declared using the k.DL annotation 'import <type> \@annotation', whose value (corresponding to
   * the name of each annotation that can tag an input in the model) is returned in the collection.
   *
   * <p>TODO revise, should be matched to a list of scanners for the observations tagged
   *
   ...
   *
   * <p>TODO revise, should be matched to a list of scanners for the observations tagged. Maybe
   * remove.
   *
