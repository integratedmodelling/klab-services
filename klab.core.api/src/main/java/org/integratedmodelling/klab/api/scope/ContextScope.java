...
/**
 * The scope for a context and any observations made within it. The context scope is the "digital
 * twin" of the observations made in it. A scope is also passed around during resolution and carries
 * scope info (namespace, project, scenarios) that is relevant to the resolver.
 * <p>
...
public interface ContextScope extends SessionScope {
    ...
    /**
     * Return a new context scope with the passed namespace of resolution. Used by the resolver or
     * to fine-tune resolution.
     * 
     * @param namespace
     * @return
     */
    ...
    /**
     * Create a context with the passed geometry to replace the one currently active. Any
     * observations made in it must be consistent with the overall geometry and context
     * observations; if observations of direct observables are made, the parent's geometry should be
    ...
    /**
     * Add another context to this one to build a higher-level one. Authentication details will
     * define what is seen and done.
     * 
     * @param remoteContext
     * @return
     */
    ...
    /**
     * Start the scheduling if the context occurs; do nothing if not. This can be called again at
     * each observation, with intelligent "replay" of any transitions that need to be seen again.
     */
    void runTransitions();
    ...
    /**
     * There is one dataflow per context, and it's never null. It starts empty and incorporates all
     * the dataflows created by the resolver when new observations are made. Each dataflow is
     * inserted in the main one at the appropriate position, so that running the dataflow again will
     * recreate the exact same context.
     * 
     * @return
     */
     ...
     * In other situations this will be null.
     * 
     * @return
     */
    ...
    /**
     * A context is born "empty" and since k.LAB 0.12 does not have a root observation, but when
     * used in resolution may acquire a root observation which serves as context for the resolution.
     * 
     * @return
     */
