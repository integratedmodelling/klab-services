 ...
 * {@link INodeIdentity} or {@link UserIdentity}. Nodes and engines start by
 * reading a certificate, which establishes the [partner, [node and [user]]]
 * identities according to context. Engines create a
 * {@link INetworkSessionIdentity} upon successful connection to the IM web, and
 * append themselves as a {@link Engine}, then duplicate the
 * {@link UserIdentity} as the default {@link IEngineUserIdentity}, to which
 * they can add others if local authentication is enabled. {@link ISession
 * Sessions} are appended to each IEngineUser, {@link Observation observations}
 * to sessions and tasks to observations.
 * <p>
 ...
 * <p>
 * Identities are arranged as follows. The chain of inheritance will depend on
 * authentication: anonymous IKlabUser users won't have a partner or node,
 * offline users will have a partner but not a node, engines that did not
 * authenticate to the network won't have a network session, etc. All these
 * conditions can be checked by calling {@link #getParentIdentity(Class)} for
 * the desired interface on the current identity and checking the result for
 * null.
 * <p>
 * Starting in k.LAB 0.11, an identity can receive a behavior and become a
 * reactive actor. The actor's capabilities depend both on the nature of the
 * identity and the behavior it receives. Identities that are created from
 * within k.IM can be bound a behavior from source code; the user-definable part
 * of behavior is specified in k.Actor, the k.LAB actor language.
 * <p>
 *
 * <pre>
 * 	IIdentity (abstract)
...
public abstract interface Identity {
	...
	/**
	 * Use to discriminate the identity - useful with
	 * {@link IRuntimeService#getRootIdentity()}.
	 * 
	 * @return
	 */
	 ...
	 *
	 * @param type a
	 *             {@link org.integratedmodelling.klab.api.services.resources.adapters.ResolutionType.IIdentity.Type}
	 *             object.
	 * @return a boolean.
	 */
