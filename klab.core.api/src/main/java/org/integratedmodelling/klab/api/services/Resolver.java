...
public interface Resolver extends KlabService {
     ...
     * This method returns a dataflow that can be executed by a runtime service. Any
     * {@link Knowledge} object can be resolved, compatibly with the observation scope. If the scope
     * is empty, the resolvable must be an {@link Instance} unless the scope has a set focal scale,
     * in which case it can be a subject observable.
     * <p>
     * This top-level method is the composition of
     * {@link #computeResolutionGraph(Knowledge, ContextScope)} and
     * {@link #computeDataflow(ResolutionGraph)}.
     * 
     * @param resolvable
     * @param scope
