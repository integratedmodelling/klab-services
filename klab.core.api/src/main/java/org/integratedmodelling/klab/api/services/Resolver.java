...
public interface Resolver extends KlabService {
	 ...
	 * context scope. This is done in two steps: resolution of the knowledge to a
	 * resolution graph (initialized from the observations in the scope) and
	 * compilatio of the resulting graph into a dataflow that can be stored or
	 * passed to a runtime service for execution.
	 * <p>
	 * Any {@link Knowledge} object can be resolved, compatibly with the observation
	 * scope and its current observation focus. If the scope is not focused on a
	 * direct observation, the resolvable must be an {@link Instance} unless the
	 * scope has a set focal scale, in which case it can be a countable observable
	 * with description type {@link DescriptionType#INSTANTIATION}.
	 * 
	 * @param resolvable
	 * @param scope
	...
	/**
	 * Compile a resolution graph into a dataflow. The scope passed must be the same
	 * that the resolution graph was computed into.
	 * 
	 * @param resolution
	 * @param scope
	 * @return
