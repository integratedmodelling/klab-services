 ...
 * workspace.
 *
 * <p>TODO use a declarative approach for all properties, so that there is one place for all default
...
public enum Configuration {
  ...
  /**
   * Development source repository on this machine. If set, a testing {@link
   * org.integratedmodelling.klab.api.engine.distribution.Distribution} will be created from the
   * Maven compiled products in it and used by the default {@link
   * org.integratedmodelling.klab.api.engine.Engine} client. Defaults to $HOME/git/klab-services.
   */
  public static final String KLAB_DEVELOPMENT_SOURCE_REPOSITORY =
      "klab.development.source.repository";
   ...
   * slash as separator. File may not exist but the paths leading to it will be created.
   *
  ...
  public String getServiceSecret(KlabService.Type serviceType) {
    File secretFile =
        Configuration.INSTANCE.getFile(
            "services/" + serviceType.name().toLowerCase() + "/secret.key");
