 ...
 *
 * <p>A model's documentation can use the \@insert tag to add project-wide templates. Each consists
 * of a set of templates, associated to triggers, each composed of a list of calls to the reporting
 ...
 *
 * <p>Documentation implementations are specific to an action language and the calling system must
...
public interface Documentation {
  ...
  /**
   * Flags for display; not used in IDocumentation at the moment, but possibly used elsewhere, e.g.
   * in {@link IPrototype#getSynopsis(Integer...)}.
   */
  ...
  public static String[] triggers =
      new String[] {
        "Initialization", "Definition", "Termination", "Instantiation", "Transition", "Event"
      };
   ...
   */
  enum Trigger {
    INITIALIZATION("Initialization", Contextualizable.Trigger.STATE_INITIALIZATION),
    DEFINITION("Definition", Contextualizable.Trigger.DEFINITION),
    INSTANTIATION("Instantiation", Contextualizable.Trigger.INSTANTIATION),
    TRANSITION("Transition", Contextualizable.Trigger.TRANSITION),
    TERMINATION("Termination", Contextualizable.Trigger.TERMINATION),
  ...
  public static interface Template {
    ...
    /**
     * Return the documentation this is part of. Needed for caching and handling of multiple model
     * incarnations.
     *
     * @return
     */
   ...
   * @param trigger
   */
  boolean instrumentReport(
      Report report, Template template, Trigger trigger, Actuator actuator, ContextScope scope);
  ...
  // are
  //     * structured in different, hierarchically organized files by ID path to minimize conflicts
  // in
  //     * Git workflows.
  //     *
  //     * @param docId
