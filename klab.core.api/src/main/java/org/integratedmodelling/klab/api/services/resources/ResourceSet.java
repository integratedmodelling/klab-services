 ...
 * resources, or reports the resources affected by a change. The returned resources should be
 * matched with anything already loaded and their versions; the order of each array is always the
 * load dependency order if one is defined. The main descriptor includes a service ID so that we can
 * establish whether the result comes from the current service or another.
 *
 * <p>Each resource descriptor lists a URN, its "knowledge class", a version and the unique ID of
 * the service that provides it, plus a {@link CRUDOperation} that details what happened to it. Each
 * service ID should be matched with the correspondent URL in the main object. Metadata may be added
 * to better describe the context of the result, provenance, authors, or rationale when appropriate.
 * The actual resources must be loaded independently; the service guarantees that any URNs mentioned
 * are served and available to the requesting identity. All ResourceSets (even if empty) must
 * contain the ID of the service that produced them. If the ResultSet is returned as the response
 * for querying a specific URN or a pattern, the result should contain the resource descriptor(s) in
 * {@link #getResults()}. All other fields contain the needed objects that must be loaded or
 * reloaded before any use of it is made.
 *
 * <p>A method isEmpty() is provided to streamline usage and possibly differentiate from a resource
 * set that contains no resources but actually represents as a non-empty result, or from an empty
 * result that does contain resources. A ResultSet can also carry notifications so that the reasons
 * for a non-expected empty set can be investigated.
 *
 * @author Ferd
 */
public class ResourceSet implements Serializable {

  @Serial private static final long serialVersionUID = 6465699208972901806L;
  ...
  /**
   * Each resource descriptor may contain metadata that describe what was done and where, or what
   * prompted a change.
   */
  public static class Resource implements Serializable {

    @Serial private static final long serialVersionUID = 4391465185554063863L;
    ...
    public String toString() {
      return Utils.Strings.capitalize(this.knowledgeClass.name().toLowerCase())
          + " "
          + this.resourceUrn
          + " v"
  ...
  /**
   * Result for any knowledge that is the focus of the query. Empty if the resource set is not the
   * result of a query for a specific URN or set thereof.
   *
   * @return
   */
  ...
  /**
   * If not null, the descriptor refers to a particular workspace. This is used particularly when
   * reporting changes.
   *
   * @return
   */
  ...
  /**
   * Return a new resource set with a result set to represent the passed resource info.
   *
   * @param info
   * @return
   */
  public static ResourceSet of(ResourceInfo info, Version version, KlabService service) {
    ...
    result.setResourceUrn(info.getUrn());
