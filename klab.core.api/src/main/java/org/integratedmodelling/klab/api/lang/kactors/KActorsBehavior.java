...
public interface KActorsBehavior extends KActorsCodeStatement, KlabAsset {
	...
	/**
	 * A serializable reference to an actor, to which we can send a message or ask a
	 * question that expects a response. Generalizes interaction with an actor using
	...
	public interface Ref extends Serializable {
		...
		/**
		 * Empty may mean that the response is null or that some agreed no-op value was
		 * set, according to implementation.
		 * 
		 * @return
		 */
		boolean isEmpty();
		...
		/**
		 * Ask a question and wait for the response. Assumes a fast response or no
		 * response; implementations should use a sensible timeout and behave as needed
		 * (null return or unchecked exception) if it's exceeded.
		 * 
		 * @param <T>
		 * @param <R>
		 * @param message
	...
	/**
	 * Return the declared locales as strings. If multiple, the locale can be set on
	 * loading.
	 * 
	 * @return
	 */
