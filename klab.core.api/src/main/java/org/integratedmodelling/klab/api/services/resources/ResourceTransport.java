...
public enum ResourceTransport {
  ...
  public static class Schema {
    ...
    public Asset asset(Map<String, Object> properties) {
      var ret = new Asset();
      ret.setProperties(Parameters.create(properties));
      return ret;
    }
    ...
    /**
     * Asset coordinate object used in service calls that provide assets for import. The asset can
     * be a direct bytestream coming from a URL or a file, or be specified through properties. A URN
    ...
    public record Property(
        String name, Artifact.Type type, boolean optional, String defaultValue) {}
    ...
    public Schema with(String property, Object defaultValue) {
      ...
      properties.put(
          property,
          new Property(
              property, Artifact.Type.classify(defaultValue), true, defaultValue.toString()));
      return this;
    }
  ...
  /**
   * Find the import schemata within those identified with <code>schemaId</code> that are compatible
   * with the passed media type and identity. If mediaType isn't null, the result should be unique.
   *
   * @param schemaId
   * @param mediaType
   * @param identity
  ...
  private List<Schema> findSchemata(
  ...
  public Schema registerImportSchema(ServiceInfo serviceInfo) {
    ...
    var schema =
        Schema.create(
            serviceInfo.getName(),
            type,
            serviceInfo.getTargets().iterator().next(),
   ...
   * components.
   *
   * <p>TODO list permissions needed to submit
   */
  ResourceTransport() {

    //        addImport("component",
    //                COMPONENT_MAVEN = Schema.create("component.maven",
    //                        Schema.Type.PROPERTIES, KlabAsset.KnowledgeClass.COMPONENT, "Register
    // a
    //                        component " +
    //                                "available on Maven " + "using " + "the component's Maven
    //                                coordinates").with(
    //                        "groupId", Artifact.Type.TEXT, false).with("adapterId",
    // Artifact.Type.TEXT,
    ...
    //                        KlabAsset.KnowledgeClass.PROJECT, "Register a k.LAB project by
    // submitting
    //                        the URL " +
    //                                "of a Git "
