...
/**
 * Dataflows in k.LAB represent "raw" computations, which can be run by a {@link RuntimeService} to
 * create, compute and link {@link Artifact}s in response to a query for the observation of some
 * {@link Knowledge} in a {@link ContextScope}. The computation is stripped of all semantics
 * (although it may report semantics in metadata for documentation and linking). Therefore it can be
 * run by a semantically-unaware workflow system, as long as the T type is a non-semantic artifact.
 * <p>
 ...
 * <p>
 * Dataflows can be serialized and rebuilt from KDL specifications. They report coverage which is
 * not the original coverage of the resolution that created them, but the <em>total</em> coverage
 * they can be computed in.
 * <p>
 * Dataflows written by users or created by k.LAB can be stored as URN-specified {@link Resource}s,
 * which can be referenced in k.LAB models.
 * <p>
 * A dataflow lists a series of top-level {@link Actuator actuator}s, which must be computed in the
 * specified order.
 * <p>
 *
...
public interface Dataflow<T extends Artifact> extends Serializable {
    ...
    /**
     * The geometry must compile to the total coverage of this dataflow, resulting from intersecting
     * the coverage of all the actuators it contains. The only situation in which it contains a
     * specific extent is when the dataflow builds an instance (but the root dataflow may union >1
     * of those). It may contain representational constraints derived from the models compiled in
     * it, such as geometry (e.g. grid) or occurrent time.
     * 
     * @return
     */
