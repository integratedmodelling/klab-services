...
/**
 * A {@code IDataArtifact} is a {@link org.integratedmodelling.klab.api.provenance.Artifact} that is
 ...
 * in a dynamic context (where time advances but the observable cannot be inferred to change in the
 * context) may only receive updates in case of event-related modifications. In such cases the state
 * may only contain the time dimensions where change has happened.
 * <p>
 *
 * @author Ferd
 * @version $Id: $Id
...
public interface DataArtifact extends Artifact {
    ...
    /**
     * Get the POD object pointed to by the locator. If the locator implies mediation, this should
     * be supported. If the locator is incompatible with the geometry, throw an exception.
     *
     * @param index a locator for the state. If the locator implies mediation, propagation or
     *        aggregation should be done.
     * @return value at index
     * @throws java.lang.IllegalArgumentException if the locator is not compatible with the
     *         artifact's geometry.
     */
