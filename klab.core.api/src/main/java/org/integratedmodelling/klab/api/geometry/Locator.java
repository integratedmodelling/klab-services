 ...
 *
 * <p>Any geometry or subset of it can be used as a locator. Scales and extents are also locators
 * and can produce their component locators as appropriate. They can also "relocate" by producing
 * lazy mediators that allow seeing observations through different lenses. When a single extent is
 * used as a locator, it will leave all the extents of any remaining others in the scale: if the
 * scale has multiple states for the locator, one or more dimensions in the located geometry will
 * have multiple states.
 *
 * <p>Locators can be parsed from a simple string parameters using the syntax below:
 *
 * <pre>
 * &lt;geometry&gt;@n,m,...
 * </pre>
 *
 * <p>where the {@link Geometry geometry} specs before @ and the @ character itself are optional if
...
public interface Locator {
  ...
  /**
   * Adapt the locator to another with the needed API. If the parameter is the class or the type of
   * an extent we want to selec the returned locator may only report location information for that
   * extent. For example, geometry.as(Space.class) will return a locator reflecting only the spatial
