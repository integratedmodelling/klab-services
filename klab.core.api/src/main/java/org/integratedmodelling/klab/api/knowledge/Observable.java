 ...
 * <p>
 * From the perspective of the k.LAB reasoner and engine, two observables are equal if and only if
 * both the semantic and the observational parts are equal. The stated name does not enter the
 * comparison.
 * <p>
 * The reference name of an observable ({@link #getReferenceName()} is as unique and stable as its
 * full URN but is a valid identifier in k.LAB. It can be used as an unambiguous identifier in code
...
public interface Observable extends Semantics {
     ...
     * <p>
     * NOTE: the builder's methods should return the same builder, not a child builder. This means
     ...
     * behavior for fluent observable specification and modification. We provide an
...
//        boolean axiomsAdded();
    ...
    /**
     * Return a builder to operate on this observable. A scope must be passed that references a
     * valid reasoner.
     * 
     * @return
     */
    ...
    /**
     * // * If true, this observable is explicitly declaring a context that is a subclass // * of
     * the "natural" context of the primary observable. For example, "X within // * RiverBasin" when
     * the natural context (declared for X) is "Region". This is // * used to speed search for
     * alternative explanations that require distributing // * calculations across different
     * objects, only done if the natural observation // * is not possible. These should only be used
     * as the observables of models, with // * full knowledge of the drawbacks (i.e., X must be
     * observable in the context // * and the observation within X must fully cover the observation
     * in the natural // * context) and the flag won't be set if the same specification is given in
     * a // * semantic declaration. // * // * @return //
     */
