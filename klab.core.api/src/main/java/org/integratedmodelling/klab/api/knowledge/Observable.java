...
public interface Observable extends Semantics {
	 ...
	 * <p>
	 * NOTE: the builder's method <em>all</em> should return the same builder, not a
	 * child builder. This means that, for example, of(concept).withTrait(trait)
	 * will apply the trait to the <em>main</em> concept, not the inherent one. In
	 * most programmatical applications, this is the desired behavior for fluent
	...
	interface Builder {
		...
		/**
		 * Call after {@link #buildConcept()} or {@link #buildObservable()} to check if
		 * any change to the ontologies were made. Returns false if the concept
		 * expression requested was already available.
		 * 
		 * @return
		 */
		boolean axiomsAdded();
		...
		/**
		 * Set the URL for the observable when it comes from a k.IM specification. Only
	...
	/**
	 * Return a builder to operate on this observable. This may not be possible if a
	 * semantic service capable of reasoning is not connected.
	 * 
	 * @return
	 */
	Builder builder();
	 ...
	 * watershed, within region). Actuators for those observations aren't scheduled
	...
	/**
	 * If true, this observable is explicitly declaring a context that is a subclass
	 * of the "natural" context of the primary observable. For example, "X within
	 * RiverBasin" when the natural context (declared for X) is "Region". This is
	 * used to speed search for alternative explanations that require distributing
	 * calculations across different objects, only done if the natural observation
	 * is not possible. These should only be used as the observables of models, with
	 * full knowledge of the drawbacks (i.e., X must be observable in the context
	 * and the observation within X must fully cover the observation in the natural
	 * context) and the flag won't be set if the same specification is given in a
	...
	/**
	 * A model URN if the observable is restricted to using a specified model to be
	 * resolved.
	 * 
	 * @return
	 */
