...
/**
 * The Interface IUserIdentity. Represents the user that runs a k.LAB session. The user can be given
 * a behavior which will intercept any actor messages directed to it or not understood by other
 * actors.
 *
...
public interface UserIdentity extends Identity, AuthenticatedIdentity {
  ...
  String FEDERATION_DATA_PROPERTY = "federation.data";

  /**
   * A user who has successfully authenticated and possesses a network session with at least one
   * online node is online. Anonymous users are always offline. This may return false and still
   * allow offline operations when users could not be authenticated because of unreachable network.
   * If the users cannot be authenticated because of a 403 response, no user identity can be
   * created.
   *
