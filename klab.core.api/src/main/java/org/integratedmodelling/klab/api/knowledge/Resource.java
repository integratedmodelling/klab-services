...
/**
 * A resource descriptor, including all the public information related to a URN-specified resource or service.
 * In the resource server this can also contain access details and credential metadata, which must be removed
 * by the adapter before the resource data are sent to any consumer. The descriptor applies to both stored
 * resources and services (the latter have " klab" as the host name in the URN).
 */
public interface Resource extends Knowledge, Resolvable {
    ...
    /**
     * Get the version associated with the resource. If the resource is a k.LAB service, return the adapter's
     * version.
     *
     ...
     *
     * @return the adapter. Should only be null when no adapter is used: resources that depend on an adapter
     * should never be created if the adapter isn't found.
     */
    String getAdapterType();

    /**
     * Resources come with both system-defined and user-defined metadata. User metadata will be indexed by
     * Dublin Core properties. Other metadata fields will depend on the adapter used (for example, no-data
     * values or metadata attributes such as name).
     *
    ...
    /**
     * Get the history of this resource as a list of all its versions. Resource services will return an empty
     * list.
     *
    ...
    /**
     * URNs coming with parameters will list them here. These are intrinsic to the resource and stored with
     * it. Parameter passed using the URN fragment are specific of the URN reference and not the resource, and
     * do not appear here.
     * <p>
     * Parameters may contain access details or credentials that must be removed by the adapter's sanitizer
     * before the resource data are sent through HTTP.
     *
    ...
    /**
     * Each resource may expose one or more attributes, which describe the content retrievable through the
     * resource. This applies to objects when the resource is of type object; use of attributes for non-object
     * resources is reserved but not operational at the moment.
     * <p>
     * The attributes should not be confused this with the resource parameters, which are stored with the
     * resource by the validator to help the encoder produce the artifacts, or with resources inputs and
     * outputs.
     *
     * @author Ferd
     */
    ...
    /**
     * A resource may have attributes beyond the "proper" ones from {@link #getAttributes()} that can be
     * categorized usefully. For example a table may need to turn the column headers into categories. These
     * are handled internally by the adapters so need no description as in an attribute, but their IDs can be
     * exposed here so that the UI can report them.
     *
     * @return
     */
    ...
    /**
     * Dependencies describe artifacts that must be available when the resource is used to produce data. They
     * use the same structure as attributes and are necessary only in URNs that identify computations.
     *
     * @return dependencies
     */
     ...
     * resource is used to produce data. They use the same structure as attributes and can only appear in URNs
     * that identify computations.
     *
     * @return dependencies
     */
    ...
    interface Attribute {
        ...
        /**
         * Return a numeric index into the resource when appropriate. This only applies to some resources that
         * have indexed attributes, so it shouldn't be relied upon unless in one of those. If the resource
         * isn't indexing its attribute, this should return -1 for all attributes.
         *
         * @return
         */
        int getIndex();
    ...
    interface Builder {
        ...
        /**
         * Add a history item. The passed resource should have no history of its own and these should be added
         * in order of timestamp, oldest first.
         *
        ...
        /**
         * Set the local resource name - for file resources, this should be the name of the primary file they
         * were loaded from, without any path but with the extension.
         *
         ...
         * externally unless we import in batch. Resource IDs don't need to check for uniqueness (the importer
         * will change suitably) but unique IDs should be set when possible, as the unique-fied built
         * externally can be ugly.
         *
         * @param identifier an identifier describing the resource, suitable for use as part of a URN (so no
         *                   colons, punctuation, uppercase characters or anything but underscores as
         *                   separators).
         */
        void setResourceId(String identifier);

        /**
         * Exclusively for use by importers that use physical files. All files set in here are copied to the
         * resource directory and exported when published.
         *
        ...
        /**
         * Add an attribute definition to the builder. For now attributes are simple enough that we just pass
         * all parameters instead of returning an attribute builder.
         *
         * @param name
         * @param type
         * @param key
        ...
        /**
         * Add a dependency definition to the builder. For now dependencies are simple enough that we just
     ...
     * resource type (e.g. in local resources it will start at the project name). May be empty, never null.
     * All paths will start with the return value of {@link #getLocalPath()}.
     *
    ...
    /**
     * If the resource is local, a local path should be defined and will identify a directory where all the
     * {@link #getLocalPaths() local file resources} are found.
     *
     * @return local path
     */
    String getLocalPath();
    ...
    /**
     * In local resources, this is the short name that the resource can be referred to in k.IM <strong>within
     * the project that owns it</strong>. It will be null in public resources and won't be recognized within
    ...
    /**
     * In local resources, this is the name of the containing project and must be valid. In public resources
     * it must be null.
     *
     * @return project name
     */
     ...
     * lowercase) must be present for each of these in the resource folder or in a public codelist repository.
     * When allowed by the providers, these can be used as authorities and their codes referenced within k.IM
     * code as identities.
     *
     * @return
     */
    ...
    /**
     * A resource may come with notifications from the validator or importer, including error notifications.
     * The online/offline status should first of all check for error notifications, which must be removed when
     * the resource is updated to fix errors.
     *
     * @return
     */
