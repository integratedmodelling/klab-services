...
public class Utils {
  ...
  public static class Lang {
    ...
    /**
     * TODO return any comment on top of the passed statement that matches the rules to be a
     * literate programming comment, or null.
     *
     * @param statement
     * @param document
     * @return
     */
    public static String getLiteralProgrammingComment(
        KlabStatement statement, KlabDocument document) {
      return null;
    }
  }
  ...
  public static class Resources {
     ...
     *
     * @param service
     * @param resources pass the actual resources to build their descriptor. Not passing anything
     *     will result in an empty resource set. If an empty array is passed the resource set will
     *     not contain resources, but the {@link ResourceSet#isEmpty()} method will return false.
     * @return
     */
    ...
    /**
     * Build a ResourceSet from a list of notifications that contain lexical scopes related to
     * document resources.
     *
     * @param notifications
     * @return
    ...
    public static ResourceSet createFromLexicalNotifications(List<Notification> notifications) {
      ...
      for (var notification : notifications) {
        if (notification.getLexicalContext() != null
            && notification.getLexicalContext().getDocumentType() != null) {

          var resource =
              resources.computeIfAbsent(
                  notification.getLexicalContext().getDocumentUrn(),
                  document -> {
                    ...
                    switch (notification.getLexicalContext().getDocumentType()) {
                      ...
                      case OBSERVATION_STRATEGY_DOCUMENT -> ret.getObservationStrategies().add(res);
    ...
    public static String dump(ResourceSet resourceSet) {
      ...
      ret.append("ResourceSet workspace=")
          .append(resourceSet.getWorkspace())
          .append(", " + "#notifications=")
          .append(resourceSet.getNotifications().size());
      boolean first = true;
      for (var resource : resourceSet.getOntologies()) {
        ret.append("\n   ")
            .append(resource.getKnowledgeClass())
            .append(" ")
            .append(resource.getResourceUrn())
            .append(" #notifications=")
            .append(resource.getNotifications().size());
      }
   ...
   * }</pre>
   *
   * <p>and type safety be damned. This will not generate any warning and will avoid any silly copy.
  ...
  public static class Paths {
     ...
     * @return the last
     */
    public static String getLast(String path, char separator) {
    ...
    public static String join(String[] pth, int start, char separator) {
      String ret = "";
      for (int i = start; i < pth.length; i++) ret += (ret.isEmpty() ? "" : ".") + pth[i];
      return ret;
    }
  ...
  public enum OS {
    ...
    /**
     * Read and return the MAC address of a machine. Can be added to capabilities (ideally in
     * encrypted form) and used to establish if server and client are on the same machine.
     *
    ...
    public static String getMACAddress() {
      ...
      try {
        for (NetworkInterface ni :
          ...
          if (adr == null || adr.length != 6) continue;
          String mac =
              String.format(
                  "%02X:%02X:%02X:%02X:%02X:%02X", adr[0], adr[1], adr[2], adr[3], adr[4], adr[5]);
          return mac;
        }
  ...
  public static class Escape {
    ...
    /**
     * Escape a url. Replaces 'invalid characters' with their Escaped code, i.e. the questionmark
     * (?) is escaped with %3F.
     *
    ...
    /**
     * Unescape a url. Replaces escapesequenced with the actual character. i.e %3F is replaced with
     * the the questionmark (?).
     *
     ...
     *
     * <p>This method exists as a defence against Cross Site Scripting (XSS) hacks. This method
     * escapes all characters recommended by the Open Web App Security Project - <a
  ...
  public static class CamelCase {
    ...
    /**
     * Converts a string to lower came case (Ex. "justASimple_String" becomes "JustASimpleString").
    ...
    private static String toCamelCaseHelper(String value, boolean upperCase, char sep) {
      ...
      char firstCharCorrected =
          (upperCase) ? toAsciiUpperCase(firstChar) : toAsciiLowerCase(firstChar);
    ...
    /**
     * Converts the specified character to upper case if it is a valid latin letter, otherwise the
     * same character is returned.
     *
     * @param ch
    ...
    /**
     * Converts the specified character to lower case if it is a valid latin letter, otherwise the
     * same character is returned.
     *
     * @param ch
     * @return the upper cased char
  ...
  public static class Numbers {
    ...
    public static boolean equal(double a, double b) {
      if (b == 0) return Double.compare(a, b) == 0;
    ...
    public static <T> List<T> podListFromString(
        String array, String splitRegex, Class<? extends T> cls) {
  ...
  public static class Strings {
    ...
    /**
     * If the string has a fragment with a separating last #, return the fragment, otherwise return
     * the string itself.
     *
     * @param uri
     * @return
     ...
     *
     * <p>If a character with value {@code searchChar} occurs in the character sequence represented
     * by the {@code cs} object at an index no smaller than {@code start}, then the index of the
     * first such occurrence is returned. For values of {@code searchChar} in the range from 0 to
     ...
     * </blockquote>
     *
     * is true. In either case, if no such character occurs inm {@code cs} at or after position
     * {@code start}, then {@code -1} is returned.
     *
     * <p>There is no restriction on the value of {@code start}. If it is negative, it has the same
     * effect as if it were zero: the entire {@code CharSequence} may be searched. If it is greater
    ...
    /**
     * Use the passed padding string to indent the first line, then indent the following ones by a
     * space pad of the same length.
     *
     * @param text
     * @param firstLinePad
    ...
    /**
     * Parse something like xxx(a, b, ... c) into an array [xxx a b c]. Accept xxx alone and send an
     * array of one element. Not particularly smart, so use when the coder is.
     *
     * @param string
     * @return
     */
     ...
     *
     * <p>For a word based algorithm, see
     * org.apache.commons.lang3.text.WordUtils#capitalize(String). A {@code null} input String
     * returns {@code null}.
     *
     * <pre>
     ...
     * @return tokens
     */
    public static Collection<String> tokenize(String s) {
     ...
     */
    public static String joinCollection(Collection<?> strings, char separator) {
      String ret = "";
     ...
     */
    public static String joinObjects(Object[] objects, char separator) {
      String ret = "";
     ...
     * @return the string
     */
    public static String joinObjects(Collection<?> objects, char separator) {
     ...
     * @return the string
     */
    public static String joinObjects(Map<?, ?> objects, char separator) {
      String ret = "";
      for (Object s : objects.keySet()) {
        ret +=
            (ret.isEmpty() ? "" : ("" + separator))
                + s
                + "="
    ...
    /**
     * Takes a multi-line string and strips the smallest amount of whitespace that is common to all
     * lines, basically removing the first level of indentation. One or more lines starting without
     * any leading whitespace will cause this to return the same string.
     *
     * @param string
     * @return
     */
     ...
     */
    public static String insertBeginning(
        String string, String patch, Function<String, Boolean> filter) {
      String lines[] = string.split("\\r?\\n");
    ...
    public static String replaceAny(
        String charsToReplace, String sourceString, String replacement) {
     ...
     * "...is the time for..."
     *
     * <p>Works like {@code abbreviate(String, int)}, but allows you to specify a "left edge"
    ...
    /**
     * Abbreviates a String using another given String as replacement marker. This will turn "Now is
     * the time for all good men" into "Now is the time for..." if "..." was defined as the
     * replacement marker.
     *
     * <p>Specifically:
     *
     ...
     *       abbrevMarker)}.
     *   <li>If {@code maxWidth} is less than {@code abbrevMarker.length + 1}, throw an {@code
    ...
    public static String abbreviate(
        final String str, final String abbrevMarker, final int maxWidth) {
      return abbreviate(str, abbrevMarker, 0, maxWidth);
    }

    /**
     * Abbreviates a String using a given replacement marker. This will turn "Now is the time for
     * all good men" into "...is the time for..." if "..." was defined as the replacement marker.
     *
     * <p>Works like {@code abbreviate(String, String, int)}, but allows you to specify a "left
     * edge" offset. Note that this left edge is not necessarily going to be the leftmost character
     * in the result, or the first character following the replacement marker, but it will appear
     ...
     *
     * <p>A negative start position can be used to start/end {@code n} characters from the end of
     * the String.
     *
  ...
  public static class Files {
    ...
    public static String getFilePath(String s) {
      ...
      int sl = s.lastIndexOf("/");
      if (sl < 0) sl = s.lastIndexOf(File.separator);
      if (sl > 0) ret = s.substring(0, sl);
    ...
    public static String getFileBaseName(String s) {
      ...
      if (sl > 0) ret = ret.substring(sl + 1);
      sl = ret.lastIndexOf(".");
      if (sl > 0) ret = ret.substring(0, sl);
    ...
    public static File createTempFile(String extension) {
      try {
        return File.createTempFile(
            "klab", (extension.startsWith(".") ? extension : ("." + extension)));
      } catch (IOException e) {
        throw new KlabIOException(e);
      }
    }
   ...
   *
   * <p>TODO not sure if these methods need synchronization. This was changed from the call so UUID
   * because under some conditions the system can run out of entropy resulting in blocking and long
   * read times.
   *
  ...
  public class Names {
    ...
    public static String getIdentifier(
        String source, int maxLength, boolean makeLowercase, Collection<String> blacklist) {
  ...
  public static class Data {
        ...
        switch (type) {
          ...
          case NUMBER:
            if (value instanceof Number) {
              return value;
            } else return Numbers.fromString(value.toString());
          case OBJECT:
            break;
          case PROCESS:
            break;
    ...
    /**
     * Flatten any collections in the source collection and accumulate values after conversion to T
     * in the destination collection.
     */
    public static <T> void collectValues(
        Collection<?> source, Collection<? extends T> destination, Class<T> class1) {
      for (Object o : source) {
    ...
    /**
     * Call the valueOf method on an enum and return the value or null if there is no such value,
     * without errors.
     *
     * @param <T>
     * @param name
    ...
    /**
     * Return the closest POD that the value can be parsed into. For now only handle int and double.
     * May add k.IM - like maps, lists, ranges.
     *
     * @param value
     * @return
     */
    ...
    /**
     * Basic conversions to match a type, including null -> NaN when what's wanted is a double or
     * float. Any value will be put into a list if a list is asked for.
     *
     * @param ret
     * @param cls
    ...
    public static <T> T asType(Object ret, Class<?> cls) {
      ...
      if (cls.isArray()
          && (ret.getClass().isArray() || Collection.class.isAssignableFrom(ret.getClass()))) {
      ...
      if (ret instanceof Number) {
        if (Number.class.isAssignableFrom(cls)) {
          ...
          if (cls.equals(Boolean.class)) {
            return (T)
                (Numbers.equal(((Number) ret).doubleValue(), 0) ? Boolean.FALSE : Boolean.TRUE);
          }
        }
      ...
      throw new KlabIllegalArgumentException(
          "cannot interpret value " + ret + " as a " + cls.getCanonicalName());
    }
  ...
  public static class URLs {
     ...
     * @throws KlabIOException the klab IO exception
     */
    public static long copy(URL url, File file, CopyListener listener, long size, int timeoutMs)
