...
public class GeometryBuilder {
  ...
  /**
   * Quickly check if the passed string looks like a WKT string in the k.LAB supported format
   * (potentially with a projection). No validation, just simple heuristics to discriminate URNs or
   * other obviously different strings.
   *
   * @param urn
   * @return
   */
  static boolean isWKT(String urn) {
    if ((urn.contains("POLYGON") || urn.contains("POINT") || urn.contains("LINESTRING"))
        && urn.contains("(")
        && urn.contains(")")) {
      return true;
    }
  ...
  public class TimeBuilder {
    ...
    public TimeBuilder extension(long[] extension) {
      // FIXME unsure this should make it a grid, or that GRID matters anyway.
      time.getParameters().put(GeometryImpl.PARAMETER_TIME_REPRESENTATION, Time.Type.GRID);
      time.getParameters().put(GeometryImpl.PARAMETER_TIME_TRANSITIONS, extension);
      return this;
    }
    ...
    public TimeBuilder year(int year) {
      ...
      end(start.plus(1, Time.Resolution.of(1, Time.Resolution.Type.YEAR)).getMilliseconds());
    ...
    public TimeBuilder resolution(Time.Resolution resolution) {
      ...
      time.getParameters()
          .put(GeometryImpl.PARAMETER_TIME_SCOPE_UNIT, resolution.getType().name().toLowerCase());
      return this;
    }
    ...
    public TimeBuilder resolution(Time.Resolution.Type resolution, double multiplier) {
      ...
      time.getParameters()
          .put(GeometryImpl.PARAMETER_TIME_SCOPE_UNIT, resolution.name().toLowerCase());
      return this;
    }
    ...
    public GeometryBuilder build() {
      if (time.getParameters().get(GeometryImpl.PARAMETER_TIME_START) == null
          || time.getParameters().get(GeometryImpl.PARAMETER_TIME_START) == null) {
        generic();
        time.getParameters().put(GeometryImpl.PARAMETER_TIME_REPRESENTATION, Time.Type.LOGICAL);
      }
  ...
  public class SpaceBuilder {
    ...
    public SpaceBuilder boundingBox(double x1, double x2, double y1, double y2) {
      space
          .getParameters()
          .put(
              GeometryImpl.PARAMETER_SPACE_BOUNDINGBOX,
              GeometryImpl.encodeVal(new double[] {x1, x2, y1, y2}));
      return this;
    }
    ...
    public SpaceBuilder resolution(Quantity gridResolution) {
      ...
      space
          .getParameters()
          .put(GeometryImpl.PARAMETER_SPACE_GRIDRESOLUTION, gridResolution.toString());
      return this;
    }
  ...
  /**
   * Create a spatial region from a resource URN (specifying a polygon). The string may also specify
   * a WKT polygon using the k.LAB conventions (preceded by the EPSG: projection). The resulting
   *
   * @param urn
   * @return
   */
   ...
   * may also specify a WKT polygon using the k.LAB conventions (preceded by the EPSG: projection).
   *
   * @param urn
  ...
  /**
   * Create a spatial grid from a lat/lon bounding box and a resolution. The box is "straight" with
   * the X axis specifying <em>longitude</em>.
   *
   * @param resolution a string in the format "1 km"
   * @return
   */
  ...
  /**
   * Create a temporal extent in years. If one year is passed, build a single-year extent; otherwise
   * build a yearly grid from the first year to the second.
   *
   * @param years
   * @return
   */
  public GeometryBuilder years(int... years) {
    if (years != null) {
      if (years.length == 1) {
        ...
        return time()
            .start(startOfYear(years[0]))
            .end(startOfYear(years[1]))
            .size((long) (years[1] - years[0]))
  ...
  /**
   * Use a builder to build a spatial geometry piece by piece. Call build() on the returned value to
   * obtain the geometry builder back. Calling build() on the space builder is not necessary for the
   * geometry to be recorded.
   *
   * @return
   */
  public SpaceBuilder space() {
