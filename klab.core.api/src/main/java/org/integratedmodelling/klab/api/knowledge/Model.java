...
public interface Model extends Knowledge, Resolvable {
  ...
  interface ResolutionInfo {
    ...
    /**
     * @return the scope of this model, with the individual model possibly redefining that of the
     *     namespace.
     */
    KlabStatement.Scope getScope();

    /**
     * @return true if the namespace this model is part of is a scenario.
     */
   ...
   * involves creating ad-hoc models that are not coming from k.IM statements. These models should
   * live in an internal namespace of the implementation choice unless {@link #inNamespace(String)}
   * is called on the builder. This builder should be usable in any other legitimate situation.
   */
  interface Builder {
  ...
  /**
   * Models are in namespaces, which are relevant to organization and scoping. Information relevant
   * to resolution coming from the namespace are available through getResolutionInfo().
   *
  ...
  /**
   * Get all information relevant to the resolver.
   * @return
   */
  ...
  /**
   * One of CONCEPT, TEXT, NUMBER, BOOLEAN or VOID if inactive because of error or offline
   * resources.
   *
  ...
  /**
   * All the observables needed by the model before contextualization. If the list has size 0, the
   * model is resolved, i.e. it can be computed without referencing any other observation.
   *
  ...
  /**
   * Models may have a coverage, either explicitly set in the model definition or in the namespace,
   * or inherited by their resources. Models with universal coverage should return an empty scale.
   * FIXME there should be a "universal" scale that isn't empty.
   *
  ...
  /**
   * If the model (or the containing namespace) is stated as <code>observed by</code> some type,
   * return it so that the model can be matched to the observer.
   *
  ...
  /**
   * The sequence of contextualizables (resources, function calls, expressions etc.) that composes
   * the computable part of the model.
   *
  ...
  /**
   * Create a builder for a model that annotates the passed resource. Call {@link
   * Builder#as(Observable)} immediately after.
   *
   * @param resource
   * @return
   */
  ...
  /**
   * Create a builder for a model that annotates the passed literal value. Call {@link
   * Builder#as(Observable)} immediately after.
   *
   * @param value
   * @return
  ...
  /**
   * Create a builder for a model that learns a resource with the passed URN. Call {@link
   * Builder#as(Observable)} immediately after.
   *
