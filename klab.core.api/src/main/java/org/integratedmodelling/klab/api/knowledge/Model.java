...
public interface Model extends Knowledge {
    ...
    /**
     * Builders are used by the resolver when creating on-the-fly models when the observation strategy involves creating
     * ad-hoc models that are not coming from k.IM statements. These models should live in an internal namespace of the
     * implementation choice unless {@link #inNamespace(String)} is called on the builder. This builder should be usable
     * in any other legitimate situation.
     */
    interface Builder {
    ...
    /**
     * The kind of description this model represents. Instantiators return {@link DescriptionType#INSTANTIATION}.
     *
     * @return
     */
    ...
    /**
     * All the observables contextualized by the model, including the "root" one that defines the model semantics.
     *
     * @return
     */
    ...
    /**
     * All the observables needed by the model before contextualization. If the list has size 0, the model is resolved,
     * i.e. it can be computed without referencing any other observation.
     *
     * @return
     */
    ...
    /**
     * Models may have a coverage, either explicitly set in the model definition or in the namespace, or inherited by
     * their resources. Models with universal coverage should return an empty scale. FIXME there should be a "universal"
    ...
    /**
     * If the model (or the containing namespace) is stated as <code>observed by</code> some type, return it so that the
     * model can be matched to the observer.
     *
     * @return
     */
    ...
    /**
     * The sequence of contextualizables (resources, function calls, expressions etc.) that composes the computable part
     * of the model.
     *
     * @return
     */
    ...
    /**
     * Create a builder for a model that annotates the passed resource. Call {@link Builder#as(Observable)} immediately
     * after.
     *
     * @param resource
     * @return
    ...
    /**
     * Create a builder for a model that annotates the passed literal value. Call {@link Builder#as(Observable)}
     * immediately after.
     *
     * @param value
     * @return
    ...
    /**
     * Create a builder for a model that learns a resource with the passed URN. Call {@link Builder#as(Observable)}
     * immediately after.
     *
