...
/**
 * An Artifact can be any of the first-class products of a k.LAB task: a non-semantic {@link
 * DataArtifact} or {@link ObjectArtifact}, an observed (semantic) {@link Observation} (as produced
 * by most activities in k.LAB) or a {@link KimModel k.IM model description} when the model has been
 * produced by an observation activity, such as a learning model.
 *
 * <p>By implementing {@link java.lang.Iterable}, we also allow Artifacts to represent groups of
 * artifacts (e.g. all subjects instantiated by resolving a subject {@link Observable observable}).
 * This enables simpler handling of provenance, as each observation activity returns one artifact,
 * possibly iterable as a group.
 *
 * <p>From an OPM perspective, IArtifact is the equivalent of Entity, including its specialization
 * into Bundle if size() > 1.
 *
 * <p>Each artifact exposes the provenance graph it's part of, allowing all k.LAB tasks to simply
...
public interface Artifact extends Provenance.Node, Iterable<Artifact> {
  ...
  enum Type {
    ...
    CONCEPT,
    /**
     * Contextualizes processes, an occurrent so the resolution only happens at transitions and not
     * at initialization. Contextualizers must have T geometry.
     */
    PROCESS,
    /**
     * The other occurrent, this time an instantiator. Instantiators must have T geometry and only
     * run at transitions.
     */
    EVENT,
    ...
    OBSERVATION,
    /**
     * Not an artifact type, but adopted by the (void) actuators that resolve acknowledged objects.
     * Must be distinct from void as the latter may make view observations. Translates directly to
     * the k.DL actuator type.
     */
    RESOLVE,

    /**
     * No value - the type of computations that resolve views and for options in command prototypes
     */
    VOID,
  ...
  interface Structure {
    ...
    /**
     * @param parent the parent artifact
     * @return a collection of logical children for the given parent, empty if no logical children
     *     exist
     */
  ...
  /**
   * Some artifact may be connected into a hierarchical structure, which may or may not be the same
   * as the natural structure of the specific type of artifact.
   *
  ...
  /**
   * The size of the group that this artifact is part of. Any artifact is part of a group including
   * at least itself.
   *
   * @return 1 or more
   */
   ...
   *
   * <p>So far used in learning models that learn contextualized observables, where no actual state
