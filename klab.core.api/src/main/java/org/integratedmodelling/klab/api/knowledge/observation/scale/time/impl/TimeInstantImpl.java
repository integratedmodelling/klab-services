...
public class TimeInstantImpl implements TimeInstant {
  ...
  public TimeInstantImpl(
    ...
    time =
        ZonedDateTime.of(
            ...
            LocalTime.of(hours, minutes, seconds, milliseconds * 1000000),
            utc);
  }
  ...
  public String toString() {
    return DateTimeFormatter.ISO_LOCAL_DATE_TIME.format(time).replace("T", " ");
  }
    ...
    switch (resolution.getType()) {
      case CENTURY:
        return new TimeInstantImpl(
            time.plusYears((int) (resolution.getMultiplier() * 100 * periods)));
      case DAY:
        return new TimeInstantImpl(time.plusDays((int) (resolution.getMultiplier() * periods)));
      case DECADE:
        return new TimeInstantImpl(
            time.plusYears((int) (resolution.getMultiplier() * 10 * periods)));
      case HOUR:
        return new TimeInstantImpl(time.plusHours((int) resolution.getMultiplier() * periods));
      case MILLENNIUM:
        return new TimeInstantImpl(
            time.plusYears((int) (resolution.getMultiplier() * 1000 * periods)));
      case MILLISECOND:
        return new TimeInstantImpl(
            getMilliseconds() + ((int) resolution.getMultiplier() * periods));
      case MINUTE:
  ...
  public TimeInstant minus(int periods, Resolution resolution) {
    switch (resolution.getType()) {
      case CENTURY:
        return new TimeInstantImpl(
            time.minusYears((int) (resolution.getMultiplier() * 100 * periods)));
      case DAY:
        return new TimeInstantImpl(time.minusDays((int) (resolution.getMultiplier() * periods)));
      case DECADE:
        return new TimeInstantImpl(
            time.minusYears((int) (resolution.getMultiplier() * 10 * periods)));
      case HOUR:
        return new TimeInstantImpl(time.minusHours((int) resolution.getMultiplier() * periods));
      case MILLENNIUM:
        return new TimeInstantImpl(
            time.minusYears((int) (resolution.getMultiplier() * 1000 * periods)));
      case MILLISECOND:
        return new TimeInstantImpl(
            getMilliseconds() - ((int) resolution.getMultiplier() * periods));
      case MINUTE:
    ...
    switch (resolution.getType()) {
      ...
      case MILLISECOND:
        return end.toInstant().toEpochMilli()
            - start.toInstant().toEpochMilli() / (int) resolution.getMultiplier();
      case MINUTE:
  ...
  public boolean isAlignedWith(Resolution res) {
    ...
    switch (res.getType()) {
      case CENTURY:
        return time.getYear() % 100 == 0
            && time.get(ChronoField.SECOND_OF_DAY) == 0
            && time.getDayOfYear() == 1;
      case DAY:
        return time.get(ChronoField.MINUTE_OF_DAY) == 0;
      case DECADE:
        return time.getYear() % 10 == 0
            && time.get(ChronoField.SECOND_OF_DAY) == 0
            && time.getDayOfYear() == 1;
      case HOUR:
        return time.getMinute() == 0;
      case MILLENNIUM:
        return time.getYear() % 1000 == 0
            && time.get(ChronoField.SECOND_OF_DAY) == 0
            && time.getDayOfYear() == 1;
      case MILLISECOND:
        return true;
  ...
  public TimeInstant beginOf(Type temporalAggregation) {
    switch (temporalAggregation) {
      ...
      case DAY:
        return new TimeInstantImpl(
            time.getYear(), time.getMonth().getValue(), time.getDayOfMonth(), 0, 0, 0, 0);
      case DECADE:
      ...
      case WEEK:
        ...
        return new TimeInstantImpl(
            monday.getYear(), monday.getMonth().getValue(), monday.getDayOfMonth(), 0, 0, 0, 0);
      case YEAR:
  ...
  public <T extends Locator> T as(Class<T> cls) {
    // TODO this should only support another TimeInstant but possibly it could be turned into Time
    // as long as a
