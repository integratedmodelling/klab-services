...
/**
 * The reasoner service collects all functionalities that use semantics in
 * k.LAB.
 */
public interface Reasoner extends KlabService {
	 ...
	 * Capabilities may depend on authentication but the endpoint should be publicly
	 * available as well.
	 * 
	 * @author Ferd
	 *
	...
	/**
	 * If the target is a union or intersection, return the operands. Otherwise
	 * return a singleton with the target itself in it.
	 * 
	 * @param target
	 * @return
	 */
	...
	/**
	 * Get a builder specified on the passed observable, used to obtain a modified
	 * observable.
	 * 
	 * 
	...
	/**
	 * For fluency. Returns a single parent for a concept known to be part of a
	 * straight hierarchy. If the concept has multiple parents, an exception is
	 * thrown.
	 * 
	 * @param c
	 * @return
	...
	/**
	 * Return the set of all children of the target, direct or indirect, using only
	 * the asserted hierarchy. For the inferred version use
	 * {@link #closure(Semantics)}.
	 * 
	 * @param target
	 * @return
	 */
	...
	/**
	 * The closure is the inferred version of {@link #allChildren(Semantics)}, which
	 * only uses the asserted hierarchy.
	 * 
	 * @param target
	 * @return
	 */
	...
	/**
	 * The core observable represented by the target, from the root namespace of the
	 * worldview.
	 * 
	 * @param first
	 * @return
	...
	/**
	 * The number of hops in the asserted hierarchy to reach the second concept
	 * starting at from. If there is no path, return -1.
	 * 
	 * @param from
	 * @param to
	 * @return
	...
	/**
	 * The "comparison" concept when the passed semantics has been modified by a
	 * unary operator that implies comparison, such as ratio, proportion or pairwise
	 * value.
	 * 
	 * @param concept
	 * @return
	...
	/**
	 * The base observable is the one that was specified in k.IM as the root of the
	 * hierarchy where the observable was specified, without any traits, modifiers
	 * or roles. Can be the concept itself.
	 * 
	 * @param observable
	 * @return
	 */
	...
	/**
	 * Remove any attribute or explicit restriction and return the raw observable,
	 * without digging down to the core definition.
	 * 
	 * @param observable
	 * @return
	 */
	...
	/**
	 * Check if concept k carries the passed trait. Uses is() on all explicitly
	 * expressed traits.
	 *
	 * @param type
	 * @param trait
	...
	/**
	 * True if the concept was declared with distributed inherency
	 * (<code>of each</code>)
	 * 
	 * @param c
	 * @return
	...
	/**
	 * Like {@link #traits(Semantics)} but only returns the traits directly
	 ...
	 *
	 * @param observable
	 * @param acceptTraits if true, will return a trait type (which can be the
	 *                     observable of a class model although it's not an
	 *                     observable per se).
	...
	/**
	 * If the passed concept is not negated, return its negation. Otherwise return
	 * the concept this has been asserted to be the negation of. If the concept is
	 * not a deniable attribute, throw an exception.
	 * 
	 * @param concept
	 * @return
	 */
	...
	/**
	 * Use the DL reasoner to check if the passed concept is semantically
	 * consistent. As all concepts are automatically checked at the time of
	 * definition, the fast way to check for consistency is
	...
	/**
	 * The knowledge domain this concept is part of, defined through the worldview.
	 * Should never be null, although currently it may be.
	 * 
	 * @param conceptImpl
	 * @return
	 */
	...
	/**
	 * Return the type described by a quality that results from applying a unary
	 * operator to it. For example
	 * <code>magnitude of earth:AtmosphericTemperature</code> describes
	...
	/**
	 * True if affecting affects affected. Uses inference when checking. Also true
	 * if the concept is a quality describing anything that is affected.
	 * 
	 * @param affected
	 * @param affecting
	 * @return
	...
	/**
	 * True if affecting creates affected. Uses inference when checking. Also true
	 * if the concept is a quality describing anything that is created or the
	 * affecting type itself (this last condition only holds for created, as the
	 * affecting type, an occurrent, must occur for its derived quality to exist).
	 * 
	 * @param affected
	 * @param affecting
	...
	/**
	 * Return the specific role that is baseRole and is implied by the context
	 * observable, either directly or through its implication closure.
	 * 
	 * @param baseRole
	 ...
	 * 
	 * @param role
	 * @param includeRelationshipEndpoints if true, roles that apply to
	 *                                     relationships will add the specialized
	...
	/**
	 * Return all the possible strategies to observe the passed observable in this
	 * context, in order of increasing cost/complexity. Except in case of abstract
	 * observables, the first should always be the direct observation of the
	 * observable without any additional computation.
	 * 
	 * @param observable
	 * @param scope
	 * @return
	...
	/**
	 * Entry point of the assisted semantic search behind interactive concept
	 * definition. If the request has a new searchId, start a new SemanticExpression
	 * and keep it until timeout or completion.
	 *
	 * @param request
	 */
	...
	interface Admin {
		 ...
		 * set. This will read all concept definitions and semantic individual
		 * definitions in the namespaces, ignoring everything else.
		 * 
		 * @param resources
		 * @return
		 */
		 ...
		 * {@link #loadKnowledge(ResourceSet, Scope)}. Provided separately to make it
		 * possible for a resolver service to declare individual local concepts, as long
		 * as it owns the semantic service. Definition must be made only in terms of
		 * known concepts (no forward declaration is allowed), so order of ingestion is
		 * critical.
		 * 
		 * @param statement
		 * @return
