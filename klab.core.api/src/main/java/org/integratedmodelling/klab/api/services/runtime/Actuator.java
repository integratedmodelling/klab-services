...
/**
 * Actuators are first-class nodes in dataflows. In other workflow systems (e.g. Ptolemy), an
 * actuator corresponds to a composite actor dedicated to an artifact and representing a k.IM model
 * (explicit or derived by the k.LAB AI). The "atomic" actors are contextualizers, represented by
 * k.LAB {@link org.integratedmodelling.ServiceCall.api.IServiceCall}s that serve as entry points
 * into the runtime, and derived from {@link Contextualizable} in k.LAB models.
 * <p>
 ...
 * base for the ID. The ID is not unique: the ID of a reference actuator must be the same of its
 * referenced one. Observations must be able to trace their way back to the actuator and the other
 * way around, without actually storing the objects, so that the dataflow can grow incrementally and
...
public interface Actuator extends Plan, Serializable {
    ...
    // *
    // * @return all imported actuators, including sub-dataflows. Use
    // * {@link #getActuators()} to only retrieve the true actuators that make
    // * up the computable set.
    // */
    // List<Actuator> getInputs();
    //
    // /**
    // * Return all the <em>computable</em> actuators in the children, thus excluding
    // * those with type RESOLVE and the sub-dataflows.
    // *
    // * @return
    // */
    ...
    // * potentially returning a new one, or the same, at the discretion of the contextualizers). It
    // * must have a first 'import' parameter and its type will match that of the filtered input.
    // *
    // * @return
    // */
    // boolean isFilter();
