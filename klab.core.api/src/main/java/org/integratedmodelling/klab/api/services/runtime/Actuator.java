...
public interface Actuator extends Plan {
    ...
    /**
     * Although actuators belong to the runtime and use no reasoning, they can have an observable,
     * which in case of pure non-semantic computations will be a non-semantic observable.
     * 
     * @return
     */
     ...
     * which should be honored. In a runtime context, the child list may contain other dataflows.
     *
     ...
     *
     * @return all imported actuators, including the sub-dataflows. Use {@link #getActuators()} to
     *         only retrieve the true actuators that make up the computable set.
     */
    List<Actuator> getInputs();

    /**
     * Return all the <em>computable</em> actuators in the children, thus excluding those with type
     * RESOLVE and the sub-dataflows.
     * 
     * @return
     */
    ...
    /**
     * Return all the dataflows in this actuator's children. These are meant to contextualize
     * objects instantiated by this one or to concretize predicates before the others are computed.
     * Dataflows loaded from serialized resolutions will not report any dataflow.
     * 
     * @return
     */
