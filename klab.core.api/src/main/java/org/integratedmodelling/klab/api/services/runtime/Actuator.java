 ...
 * Within an actuator, the individual runtime operations are represented by k.LAB
 * {@link ServiceCall}s that serve as entry points into the runtime, and derived from
 * {@link Contextualizable} in k.LAB models. Before computing an actuator, the runtime must ensure
 * that all the components serving the needed service calls are available, authorized and loaded,
 * interacting appropriately with the {@link ResourcesService}s in the scope. An actuator with no
 * computation is just responsible for creating its observation, and only non-dependent observables
 * ("substantials", generating {@link DirectObservation}s) have the ability to just "acknowledge"
 * the observation without providing a computation strategy.
 * <p>
 * Actuators may be references, corresponding to "input ports" in other workflow systems and used
 ...
 * base for the ID. The ID is not unique as the ID of a reference actuator must be the same of its
 * referenced one, but it uniquely identifies an observable and its observation. Observations must
 * be able to trace their way back to the actuator and the other way around, without actually
 * storing the objects, so that the dataflow can grow incrementally with successive queries and a
 * coherent provenance graph can be maintained.
 *
...
public interface Actuator extends Plan, Serializable {
     ...
     * associated with the semantics or to its renaming through 'named' when used as a dependency.
     *
     ...
     * actuators resolve views or predicates. Actuators whose type defines an occurrent are not run
     * at initialization but are just called upon to schedule temporal actions.
     * 
     * @return
     */
    ...
    /**
     * If the actuator is deferred, the observable is not resolved but must be resolved within the
     * parent actuator (the deferring actuator) once the latter has produced observations. If the
     * deferring actuator is for a concrete direct observable, the deferred observable will be
     * resolved within each instance; if it's for an abstract observable, the deferred observable
     * will be substituted with the results of resolving each concrete observable resulting from its
     * resolution, potentially applying its computations to merge the results.
     * 
     * @return
     */
    boolean isDeferred();
     ...
     * that should be honored by serial runtimes.
     * <p>
     * In a runtime context, the child list may contain actuators that serve as inputs, outputs,
     ...
     * artifact from the computation context.
     * 
     * @return
     */
    boolean isOutput();
     ...
     * contextualization. It only serves as a placeholder with a possibly different alias to define
     * the local identifier of the original observation. Reference actuators are otherwise empty,
     * with no children and no computation.
     * 
     * @return
     */
    boolean isReference();
     ...
     * actuator. Dealing with different coverages within a model is the responsibility of the
     * runtime. A null or empty coverage returned here means universal coverage, as no actuators are
     * output by a resolution that does not succeed.
     * 
    ...
    /**
     * The observer on behalf of whom the observations are made. If null, the "objective" point of
     * view of the current session user is meant.
     * 
     * @return
     */
    String getObserver();
