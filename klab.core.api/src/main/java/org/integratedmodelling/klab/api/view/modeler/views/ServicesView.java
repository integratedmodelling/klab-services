...
public interface ServicesView extends View {
  ...
  /**
   * Called after a change in service configuration has been made, passing the service that has
   * changed status and the new running status for it. The UI should list all the services available
   * in the engine and show the status of the current one(s) per each monitored category. If there
   * wasn't a current service at the time of the call, the view should call {@link
   ...
   * as if the user had selected the service, so that the current service in the engine is
   * guaranteed in sync with what is shown in the view.
   */
