...
class FillCurveTest {
  ...
  void offset_rowMajor_linear_and_XY_and_XYZ() {
    ...
    for (int s = -10; s <= 20; s++) {
      ...
      assertEquals(expected, Data.FillCurve.D1_LINEAR.offset(s, size1));
    }
    ...
    for (int s = -5; s <= 10; s++) {
      long total = 3L * 2L;
      long expected = ((s % total) + total) % total;
      assertEquals(expected, Data.FillCurve.D2_XY.offset(s, size2));
    }
    ...
    for (int s = -limit; s <= limit; s++) {
      long expected = ((s % total) + total) % total;
      assertEquals(expected, Data.FillCurve.D3_XYZ.offset(s, size3));
    }
  }
  ...
  void offset_YX_and_XInvY_examples() {
    ...
    for (int s = 0; s < expectedYX.length; s++) {
      assertEquals(expectedYX[s], Data.FillCurve.D2_YX.offset(s, sizeXY), "D2_YX step " + s);
    }

    long[] sizeXInvY = new long[] {2, 3};
    long[] expectedXInvY = new long[] {2, 1, 0, 5, 4, 3};
    for (int s = 0; s < expectedXInvY.length; s++) {
      assertEquals(expectedXInvY[s], Data.FillCurve.D2_XInvY.offset(s, sizeXInvY), "D2_XInvY step " + s);
    }
  }
...
//  }
  ...
  void hilbert_variants_throw_in_offset_and_map() {
    ...
    assertThrows(UnsupportedOperationException.class, () -> Data.FillCurve.D2_HILBERT.offset(0, size2));
    assertThrows(UnsupportedOperationException.class, () -> Data.FillCurve.D3_HILBERT.offset(0, size3));

    assertThrows(UnsupportedOperationException.class, () -> Data.FillCurve.D2_HILBERT.map(0, size2, Data.FillCurve.D2_XY));
    assertThrows(UnsupportedOperationException.class, () -> Data.FillCurve.D3_HILBERT.map(0, size3, Data.FillCurve.D3_XYZ));
  }

  @Test
  void invalid_sizes_rejected() {
    // Null or empty sizes
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D1_LINEAR.offset(0, null));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D1_LINEAR.offset(0, new long[] {}));

    // Zero or negative size
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.offset(0, new long[] {3, 0}));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.offset(0, new long[] {3, -2}));

    // Dimensionality mismatch
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.offset(0, new long[] {3}));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D3_XYZ.offset(0, new long[] {2, 2}));

    // Also check map validations
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.map(0, null, Data.FillCurve.D2_YX));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.map(0, new long[] {}, Data.FillCurve.D2_YX));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.map(0, new long[] {3, 0}, Data.FillCurve.D2_YX));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.map(0, new long[] {3}, Data.FillCurve.D2_YX));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.map(0, new long[] {3, 2}, null));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.map(0, new long[] {3, 2}, Data.FillCurve.D3_XYZ));
  }
  ...
  void offsets_rowMajor_linear_and_XY_and_XYZ() {
    ...
    for (int s = -7; s < 12; s++) {
      ...
      long[] c = Data.FillCurve.D1_LINEAR.offsets(s, size1);
    ...
    long total2 = 12;
    for (int s = -3; s < 20; s++) {
      long[] coords = Data.FillCurve.D2_XY.offsets(s, size2);
    ...
    long total3 = 12;
    for (int s = -13; s < 25; s++) {
      long[] coords = Data.FillCurve.D3_XYZ.offsets(s, size3);
  ...
  void offsets_consistency_with_offset_for_variants() {
    ...
    for (int s = 0; s < total; s++) {
      long[] cYX = Data.FillCurve.D2_YX.offsets(s, sizeXY);
      assertEquals(
              Data.FillCurve.D2_YX.offset(s, sizeXY),
    ...
    for (int s = 0; s < total2; s++) {
      long[] c = Data.FillCurve.D2_XInvY.offsets(s, sizeXInvY);
      assertEquals(
              Data.FillCurve.D2_XInvY.offset(s, sizeXInvY),
    ...
    long[] size3 = new long[] {2, 2, 2};
    for (int s = 0; s < 8; s++) {
      long[] c = Data.FillCurve.D3_ZYX.offsets(s, size3);
      assertEquals(
              Data.FillCurve.D3_ZYX.offset(s, size3),
  ...
  void offsets_specific_examples_XInvY() {
    ...
    for (int s = 0; s < expected.length; s++) {
      long[] c = Data.FillCurve.D2_XInvY.offsets(s, size);
  ...
  void offsets_hilbert_and_invalid_sizes() {
    ...
    assertThrows(
        ...
        () -> Data.FillCurve.D2_HILBERT.offsets(0, size2));
    assertThrows(
        UnsupportedOperationException.class,
        () -> Data.FillCurve.D3_HILBERT.offsets(0, size3));

    // invalid sizes
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D1_LINEAR.offsets(0, null));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D1_LINEAR.offsets(0, new long[] {}));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.offsets(0, new long[] {3, 0}));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.offsets(0, new long[] {3, -1}));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D2_XY.offsets(0, new long[] {3}));
    assertThrows(IllegalArgumentException.class, () -> Data.FillCurve.D3_XYZ.offsets(0, new long[] {2, 2}));
  }
}
