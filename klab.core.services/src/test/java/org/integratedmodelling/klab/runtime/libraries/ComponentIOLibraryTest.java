...
class ComponentIOLibraryTest {
    ...
    @Mock
    private ComponentRegistry registry = Mockito.mock(ComponentRegistry.class);

    @BeforeEach
    void init() {
        Mockito.when(service.getComponentRegistry()).thenReturn(registry);
        Mockito.when(registry.installComponent(any(), ArgumentMatchers.anyString())).thenReturn(Pair.of(null, ResourceSet.of(new ResourceInfo(), Version.ANY_VERSION)));
    }
