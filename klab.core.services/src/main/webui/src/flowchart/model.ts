import type { ElkNode, ElkExtendedEdge, ElkPort, ElkLabel } from "elkjs/lib/elk-api";
...
export interface FlowChart { root: ChartNode; metadata?: Metadata }
export interface DiagramSelection { id: string; kind: string; metadata: Metadata }
...
export async function layoutChart(chart: FlowChart, signal?: AbortSignal): Promise<FlowChart> {
  ...
  function prepare(node: ChartNode) {
    ...
    node.layoutOptions = { "elk.padding": "[top=36,left=24,bottom=24,right=24]",
      "elk.nodeLabels.placement": "[H_CENTER,V_TOP,INSIDE]", "elk.portConstraints": "FIXED_SIDE", ...node.layoutOptions };
    for (const port of node.ports || []) {
      port.width = Math.max(8, port.width || 0); port.height = Math.max(8, port.height || 0);
      port.layoutOptions = { "elk.port.side": port.role === "INPUT" ? "WEST" : "EAST", ...port.layoutOptions };
      measure(port.labels);
    }
    for (const edge of node.edges || []) { delete edge.sections; measure(edge.labels); }
    node.children?.forEach(prepare);
  }
  prepare(copy.root);
  copy.root.layoutOptions = { "elk.algorithm": "layered", "elk.direction": "RIGHT",
  ...
  try {
    const pending = elk.layout({ id: `layout-${crypto.randomUUID()}`,
    ...
    copy.root.x = 0; copy.root.y = 0;
  }
  finally { if (abort) signal?.removeEventListener("abort", abort); elk.terminateWorker(); }
  return copy;
}
...
export function toSprotty(chart: FlowChart): { model: SGraph; elements: Map<string, DiagramSelection> } {
  ...
  function node(value: ChartNode, x: number, y: number) {
    ...
    const ports = (value.ports || []).map(port => {
      ...
      return { type: "port", id: port.id, cssClasses: [port.role?.toLowerCase() || "input"],
        position: { x: port.x || 0, y: port.y || 0 }, size: { width: port.width || 8, height: port.height || 8 },
        children: labels(port.labels) };
    });
    children.push({ type: "node", id: value.id, position: { x, y },
    ...
    for (const edge of value.edges || []) {
      ...
      children.push({ type: "edge", id: edge.id, sourceId: edge.sources[0], targetId: edge.targets[0],
