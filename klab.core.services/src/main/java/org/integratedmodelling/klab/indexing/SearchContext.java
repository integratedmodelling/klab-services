...
public class SearchContext {
      ...
      if (match != null) {
        switch (match.getMatchType()) {
          ...
          case PRESET_OBSERVABLE:
            Concept concept =
                scope.getService(Reasoner.class).resolveObservable(match.getId()).getSemantics();
            if (concept != null) {
              if (concept.is(SemanticType.OBSERVABLE)) {
                this.observable = concept;
                this.semantics = EnumSet.copyOf(concept.getType());
              } else if (concept.is(SemanticType.PREDICATE)) {
  ...
  static class Condition {
    ...
    public String toString() {
      return "[CONDITION "
          + type
          + " "
          + (semantics == null ? "" : semantics.toString())
          + (c1 == null ? "" : (" C1: " + c1))
          + (c2 == null ? "" : (" C2: " + c2))
          + "]";
    }
  ...
  /**
   * Constraints can be matchers (which produce matches directly) or queries (which produce queries
   * for documents). Optionally, they may also be filters (which scan the query results and
   * accept/reject them selectively). Conditions of various types can be added to build intelligent
   * filters.
   *
  ...
  static class Constraint {
    ...
    public String toString() {
      ...
      ret +=
          matcher
              ? " MATCH"
              : (type == SemanticMatch.Type.MODIFIER
                  ? (modifiers == null ? "[ALL]" : modifiers.toString())
                  : "");
    ...
    public boolean filter(SemanticMatch match) {
      ...
      if (!allowAbstract && match.getSemantics() != null) {
        if (match.getSemantics().contains(SemanticType.ABSTRACT)
            ...
            && !match.getSemantics().contains(SemanticType.PREDICATE)) {
          return false;
        }
      }
    ...
    public static Constraint allTraits(boolean allowAbstract) {
      Constraint ret = new Constraint(SemanticMatch.Type.CONCEPT);
      ret.semantics = EnumSet.of(SemanticType.PREDICATE);
      ret.query = true;
      ret.filter = true;
      ret.allowAbstract = allowAbstract;
      return ret;
  ...
  private SearchContext acceptToken(SemanticMatch match) {
    ...
    if (match.getUnaryOperator() != null) {
      ...
      switch (match.getUnaryOperator()) {
        ...
        case NOT:
          ret.allow(
              Constraint.allTraits(false)
                  .applyingTo(SemanticType.ATTRIBUTE)
                  .applyingTo(SemanticType.DENIABLE));
          break;
        case OCCURRENCE:
        ...
        case PROPORTION:
          ...
          ret.allow(Constraint.with(SemanticType.PREDICATE));
          ret.allow(Constraint.allTraits(false).applyingTo(SemanticType.PREDICATE));
          break;
        case RATIO:
      ...
      switch (match.getModifier()) {
        ...
        case BY:
          ret.allow(
              Constraint.with(SemanticType.QUALITY_TYPES)
                  .plus(SemanticType.COUNTABLE, SemanticType.PREDICATE)
        ...
        case CONTAINING:
          ...
          break;
        case DOWN_TO:
          ret.allow(Constraint.with(SemanticType.CLASS, SemanticType.PREDICATE));
          ret.allow(Constraint.allTraits(false) /* .applyingTo(SemanticType.EVENT) */);
          break;
        case DURING:
          ret.allow(Constraint.with(SemanticType.EVENT));
        ...
        case IN:
          ret.nextTokenType =
              meaning.getSemantics().contains(SemanticType.MONEY)
                      || meaning.getSemantics().contains(SemanticType.MONETARY_VALUE)
                  ? TokenClass.CURRENCY
                  : TokenClass.UNIT;
      ...
      if (meaning.getObservable() != null) {
        ...
        ret.allow(
            Constraint.otherTraits(meaning.getTraits(), scope).applyingTo(meaning.getSemantics()));
  ...
  public static SearchContext createNew(
      Set<SemanticMatch.Type> matchTypes, Set<SemanticType> semanticTypes, Scope scope) {
    SearchContext ret = new SearchContext(scope);
