...
/**
 * This class implements bin operations (insertions, merges, etc) for a
 * histogram. This implementation is best for histograms with a small (<=256)
 * number of bins. It uses an ArrayList to give O(N) insert performance with
 * regard to the number of bins in the histogram. For histograms with more bins,
 * the TreeBinReservoir class offers faster insert performance.
 */
	...
	public void insert(Bin<T> bin) {
		...
		if (index >= 0) {
			...
			if (isFrozen() && _bins.size() == getMaxBins()) {
				...
				double prevDist = (prevIndex >= 0) ? bin.getMean() - _bins.get(prevIndex).getMean() : Double.MAX_VALUE;
				double nextDist = (nextIndex < _bins.size()) ? _bins.get(nextIndex).getMean() - bin.getMean()
						: Double.MAX_VALUE;
