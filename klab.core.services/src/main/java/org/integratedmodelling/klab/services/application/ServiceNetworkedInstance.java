 ...
 * served as an online service through REST endpoints handled by Spring controllers. The base
 * packages scanned below should be ALSO present in the final derived class, which should add its
 * own. That way all services can use the same security model and present the same common API.
 *
 * <p>The {@link org.integratedmodelling.klab.api.scope.ServiceScope} is produced using a service
 * certificate located in the configured service dataspace. If that isn't available, the service
 * will use the strategy from the superclass and will only be available within the local network if
 * the user is authenticated, or the local machine if anonymous.
 */
@Component
...
public abstract class ServiceNetworkedInstance<T extends BaseService> extends ServiceInstance<T>
  ...
  private String description =
      "API documentation for the k.LAB reasoner service. POST methods use valid "
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    super.start(
        environment.getRequiredProperty("klab.service.options", ServiceStartupOptions.class));
  }

  /**
   * Call this in main() using the concrete subclass of ServiceNetworkedInstance and the desired
   * startup options.
   *
   * @param cls
   * @param options
            ...
            propName -> {
              if (propName.contains("klab.")) {
                ServiceConfiguration.INSTANCE
                    .getProperties()
                    .setProperty(propName, environment.getProperty(propName));
              }
            });
  }
