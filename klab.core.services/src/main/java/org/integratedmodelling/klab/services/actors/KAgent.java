 ...
 * {@link org.integratedmodelling.klab.api.services.runtime.Message}s with a
 * {@link org.integratedmodelling.klab.api.services.runtime.Message.MessageClass} of
 * {@link org.integratedmodelling.klab.api.services.runtime.Message.MessageClass#ActorCommunication}. The
 * {@link Ref actor reference} is available through the context; to communicate with the agent, it's enough to
 * use {@link org.integratedmodelling.klab.api.services.runtime.Channel#send(Object...)} on the scope that
 * owns the agent using message class
 * {@link org.integratedmodelling.klab.api.services.runtime.Message.MessageClass#ActorCommunication}.
 * <p>
 * Each agent can have its own k.Actors VM which runs all the behaviors sent concurrently in separate threads.
 * The agent still processes any messages sent to it (explicitly or through behavior execution) sequentially.
 * Each run uses a run scope that contains the actor ref and the Scope that the agent was created in.
 * <p>
 * In this implementation, users, sessions/scripts/applications, top-level contexts and individual
 * observations bound to behaviors wrap an agent and can run k.Actors behaviors.
 *
...
public abstract class KAgent implements ReActor {
    ...
    public static class KAgentRef implements Ref {
        ...
        @Override
        public <T extends Serializable, R extends Serializable> R ask(T message,
     ...
     * overridden if needed (call super.setBehavior()!) to handle more messages, but overriding
     * {@link #handleMessage(ReActorContext, Message)} should serve most purposes.
     *
     * @return
     */
    ...
    protected void handleMessage(ReActorContext reActorContext, Message message) {
        if (message.getMessageType() == Message.MessageType.RunBehavior) {
            ...
            if (behavior != null) {
                ...
                scope.error("Behavior could not be identified from message with content " + message.getPayload(String.class));
            }
        }
    }
    ...
    /**
     * Extend to provide initialization. MUST call super.initialize()! NOTE: when this is called, the scope is
     * defined but it does not contain the agent ref yet. Use self only.
     *
     * @param rctx
     * @param message
     */
