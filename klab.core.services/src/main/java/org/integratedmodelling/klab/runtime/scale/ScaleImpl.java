...
public class ScaleImpl implements Scale {
   ...
   * maintains the offset[s] and the index[es] of the extend that is changing. This is the default
   * locator if the scale hasn't been adapted to one of the fast scanners.
   *
   * <p>TODO make 2 more that consider masking in extents
   *
   * @author Ferd
   */
  abstract class ScaleLocator extends DelegatingScale {

    @Serial private static final long serialVersionUID = 797929992176158102L;
    long offset;
  ...
  /**
   * Internal locator class for the situation where a single dimension is changing and the others
   * are locked at a specified extent.
   *
   * @author Ferd
   */
  class ScaleLocator1D extends ScaleLocator {

    @Serial private static final long serialVersionUID = -4207775306893203109L;
    int changingIndex;
    long[] extents;
  ...
  public Geometry merge(Scale other, LogicalConnector how) {
    ...
    List<Extent<?>> ret = new ArrayList<>();
    Set<Dimension.Type> commonConcepts = EnumSet.noneOf(Dimension.Type.class);
    Set<Dimension.Type> addedConcepts = EnumSet.noneOf(Dimension.Type.class);

    for (var e : extents) {
      var common = other.extent(e.getType());
      if (common != null) {
        ret.add(e.merge(common, how));
      } else {
        ret.add(e);
      }
      addedConcepts.add(e.getType());
    }

    for (var e : other.getExtents()) {
      if (!addedConcepts.contains(e.getType())) {
        ret.add(e);
      }
    }

    return new ScaleImpl(ret);
  }
  ...
  public Scale initialization() {
    ...
    return new ScaleImpl(
        Arrays.stream(extents)
            .map(e -> e.getType() == Type.TIME ? ((Time) e).initialization() : e)
  ...
  public Scale termination() {
    ...
    return new ScaleImpl(
        Arrays.stream(extents)
            .map(e -> e.getType() == Type.TIME ? ((Time) e).termination() : e)
            .collect(Collectors.toList()));
  }
  ...
  public Scale without(Type dimension) {
    ...
    var extents = new Extent[this.extents.length - 1];
    long size = 1;
    int next = 0;
