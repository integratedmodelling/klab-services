...
public class JsonValueHelper {
  private static final DateTimeFormatter format = Utils.Time.getResilientFormatterAtUTC();
  ...
  /**
   * Tries to "guess" the typed value of a node. It resolves only dates and dates/times to their
   * actual Java type. For other type of values, it returns the string value of the node.
   *
   * @param node The JSON node
   */
  public static Object tryGuessTypedValue(TreeNode node) {
    ...
    if (datePattern.matcher(value).find() && value.length() == 10) {
      ...
      return LocalDateTime.parse(value, Utils.Time.getResilientFormatterAtUTC());
    } else {
      return value;
    }
  }
