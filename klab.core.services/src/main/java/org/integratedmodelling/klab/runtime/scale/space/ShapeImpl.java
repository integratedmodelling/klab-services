...
public class ShapeImpl extends SpaceImpl implements Shape {
  ...
  public Shape simplifyIfNecessary(int maxCoordinates, int nDivisions) {
    if (geometry.getNumPoints() > maxCoordinates) {
      double distance =
          Math.sqrt(Math.pow(getEnvelope().getWidth(), 2) + Math.pow(getEnvelope().getHeight(), 2))
              / (double) nDivisions;
  ...
  /**
   * Join the passed shapes into another shape of the passed type, optionally including or excluding
   * the shapes themselves.
   *
   * <p>Assumes both shapes have the same projection.
   *
   * @param a
   * @param b
   * @param type
      ...
      case LINESTRING:
        merged =
            aj.getFactory()
                .createLineString(
                    new Coordinate[] {
                      aj.getCentroid().getCoordinates()[0], bj.getCentroid().getCoordinates()[0]
                    });
        break;
      case POLYGON:
      case MULTIPOLYGON:
  ...
  public double[] getCenter(boolean standardized) {
    Point centroid =
        standardized ? getStandardizedGeometry().getCentroid() : geometry.getCentroid();
