...
public class HttpClient extends SimpleHttpClient {
    ...
    if (this.authentication != null) {
      ...
      try (DataOutputStream out = new DataOutputStream(connection.getOutputStream())) {
        final String content =
            "user="
                + URLEncoder.encode(getUser(), "UTF-8")
                + "&password="
      ...
      final Object data = new ObjectMapper().reader().readValue(builder.toString());
  ...
  private HttpsURLConnection applySelfSignedLocalhostFix(HttpsURLConnection connection)
