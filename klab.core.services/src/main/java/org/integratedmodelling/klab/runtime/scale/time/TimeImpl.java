...
public class TimeImpl extends ExtentImpl<Time> implements Time {
  ...
  @Serial private static final long serialVersionUID = 5936628543110335175L;
  ...
  public Time at(Locator locator) {
    ...
    // locators[0]).withScaleId(getScaleId()).withLocatedOffset(0);
    ...
    //                } else {
    //                    /*
    //                     * Mediation situation. Because of the irregular extents, not doing the
    // coverage
    ...
    //                        if (last.getStart().getMilliseconds() >= ((ITimeInstant)
    // locators[0]).getMilliseconds()
    //                                || last.getEnd().getMilliseconds() > ((ITimeInstant)
    // locators[0]).getMilliseconds()) {
    ...
    //                        // long nn = n;
    //                        // // previous was approximate due to potential irregularity; correct
    // as
    //                        // // needed
    //                        // while(nn > 0 && ret.getEnd().isBefore(((ITimeInstant)
    // locators[0]))) {
  ...
  public String encode() {
    ...
    if (target.getStart() != null) {
      if (target.getEnd() != null) {
        ...
        args +=
            "," + GeometryImpl.PARAMETER_TIME_LOCATOR + "=" + target.getStart().getMilliseconds();
      }
    }
    if (target.step != null) {
      args +=
          "," + GeometryImpl.PARAMETER_TIME_GRIDRESOLUTION + "=" + target.step.getMilliseconds();
    }
    ...
    if (target.coverageResolution != null) {
      args +=
          ","
              + GeometryImpl.PARAMETER_TIME_COVERAGE_UNIT
              + "="
              + target.coverageResolution.getType();
  ...
  public boolean isEmpty() {
    return size() == 0 || start == null || end == null;
  }
  ...
  public boolean distributed() {
    return getTimeType() == Time.Type.GRID
        || getTimeType() == Time.Type.REAL
        || (this.extension != null && this.extension.size() > 1);
  }
  ...
  public static Time create(Dimension dimension, Scope context) {
    ...
    String representation =
        dimension.getParameters().get(GeometryImpl.PARAMETER_TIME_REPRESENTATION, String.class);
    Double scope = dimension.getParameters().get(GeometryImpl.PARAMETER_TIME_SCOPE, Double.class);
    String unit =
        dimension.getParameters().get(GeometryImpl.PARAMETER_TIME_SCOPE_UNIT, String.class);
    Long locator = dimension.getParameters().get(GeometryImpl.PARAMETER_TIME_LOCATOR, Long.class);
    String res =
        dimension.getParameters().get(GeometryImpl.PARAMETER_TIME_GRIDRESOLUTION, String.class);
    ...
    String cunit =
        dimension.getParameters().get(GeometryImpl.PARAMETER_TIME_COVERAGE_UNIT, String.class);
    Long cstart =
        dimension.getParameters().get(GeometryImpl.PARAMETER_TIME_COVERAGE_START, Long.class);
      ...
      if (resolution != null) {
        if (resolution.getType().isRegular()) {
          this.size =
              (long)
                      ((end.getMilliseconds() - start.getMilliseconds())
                          / (resolution.getType().getMilliseconds() * resolution.getMultiplier()))
                  + 1;
        } else {
          // compute on request
          this.regular = false;
