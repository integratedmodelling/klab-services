...
import org.integratedmodelling.klab.runtime.data.stac.model.*;
import org.integratedmodelling.klab.runtime.data.stac.parser.STACParser;
...
/**
 * Client for a STAC web service. It allows for: - browsing various STAC items (catalog,
 * collections, items) - searching collections - downloading individual assets or full items
 *
 * @author Cosmin Cara
 */
public class STACClient {
  ...
  /**
   * Initializes a new client for the given URL, with no authentication.
   *
   * @param baseURL The URL of the STAC web service
   */
  public STACClient(String baseURL) throws MalformedURLException {
  ...
  public Collection getCollection(String collectionName) throws IOException {
    final HTTPResponse response =
        this.client.get(new URL(this.stacURL + "/collections/" + collectionName));
    try (InputStream inStream = response.getResponseStream()) {
   ...
   */
  public ItemCollection listItems(String collectionName, int pageNumber, int pageSize)
      throws IOException {
   ...
   */
  public ItemCollection search(String collectionName, Map<String, Object> parameters)
      throws IOException {
   ...
   * @param pageSize The page size
   */
  public ItemCollection search(
      String collectionName, Map<String, Object> parameters, int pageNumber, int pageSize)
      throws IOException {
    if (parameters == null) {
  ...
  public void download(Asset asset, Path folder) throws IOException {
    ...
    try (ReadableByteChannel inputChannel = Channels.newChannel(response.getResponseStream());
        WritableByteChannel outputChannel =
            Channels.newChannel(Files.newOutputStream(folder.resolve(name)))) {
