...
/**
 * Runtime scope for all k.Actors statements. Root scopes are for each action. Local class so that the
 * identity is accessible.
 *
 * @author Ferd
 */
public class KActorsScope implements VM.BehaviorScope {
    ...
    /**
     * The scope is functional if an action that is declared as 'function' is called, or if the executing
     * action is part of a contextual chain (a1().a2().a3: ...). In this case any "fire" statement does not
     * fire a value but "returns" it, setting it in the scope and breaking the execution.
     */
    ...
    /**
     * Get a child scope for this action, which will create a panel viewscope if the action has a view.
     *
     * @param action
     * @return
     */
    ...
    /**
     * The scope for a child component detaches both local and global symbols to keep them local to the
     * child.
     *
     * @return
     */
...
//    }
    ...
    public Channel onEvent(Message.MessageClass messageClass, Message.MessageType messageType,
        ...
        return mainScope.onEvent(messageClass, messageType, runnable, matchArguments);
    }
