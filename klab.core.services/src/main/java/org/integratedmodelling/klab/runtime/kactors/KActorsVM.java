...
/**
 * The basic k.Actors virtual machine (VM). Each actor should own a VM but components will be
 * routines, not actors, thereby saving the whole mess of forward IDs. The VM must be fully
 * reentrant w.r.t. behaviors, actions and state.
 *
 * <p>VMs should be usable without an actor as long as no actor-specific calls are made. These
 * should be aware that the actor can be null and terminate gracefully or warn and move on.
 * Communication with the actor is handled by scope.send(), which in agent-enabled scopes must
 * filter messages that implement VM.AgentMessage and tell() them to the agent Ref instead of
 * sending them along the normal route.
 *
...
public class KActorsVM implements VM {
  ...
  public static final String JAVA_EXTENSION_NAMESPACE =
      "org.integratedmodelling.klab.components.runtime" + ".actors.extensions";
  ...
  public void runBehavior(
      KActorsBehavior behavior, Parameters<String> arguments, KActorsScope scope) {
    ...
    new Thread() {
      ...
      public void run() {
        ...
        try {
          ...
          boolean rootView =
              scope.getViewScope() == null ? false : (scope.getViewScope().getLayout() == null);
  ...
  protected void run(KActorsAction action, KActorsBehavior behavior, KActorsScope scope) {
    ...
    if (wspecs != null) {
      ...
      scope.send(
          ...
          "modal".equals(wspecs.getName())
              ? Message.MessageType.CreateModalWindow
  ...
  private void executeInstantiation(
      Instantiation code, KActorsBehavior behavior, KActorsScope scope) {
    ...
    String actorName =
        code.getActorBaseName() + (actors == null ? "" : ("_" + (actors.size() + 1)));
    ...
    if (!code.getActions().isEmpty()) {
      ...
      for (Triple<KActorsValue, KActorsStatement, String> adesc : code.getActions()) {
        actions.matches.add(
            Pair.of(new Match(adesc.getFirst(), adesc.getThird()), adesc.getSecond()));
      }
  ...
  private void executeAssert(Assert code, KActorsBehavior behavior, KActorsScope scope) {
    ...
    for (Assertion assertion : code.getAssertions()) {
      ...
      if (assertion.getValue() != null || assertion.getExpression() != null) {
        ...
        evaluateAssertion(
            scope.getMatchValue() == null ? scope.getValueScope() : scope.getMatchValue(),
            assertion,
            scope,
            code.getArguments());
      }
  ...
  private static Object executeFunctionChain(
      List<Call> functions, KActorsBehavior behavior, KActorsScope scope) {
    ...
    for (int i = 0; i < functions.size(); i++) {
      ...
      KActorsScope fscope =
          last ? scope.withReceiver(contextReceiver) : scope.functional(contextReceiver);
  ...
  /**
   * A call sequence is a one or more calls to be executed in sequence. The last call is a standard
   * message call which will either fire or return according to the scope; the ones preceding it, if
   * any, are necessarily functional and the return value of the first provides the execution
    ...
    switch (arg.getType()) {
      ...
      case ERROR:
        throw arg.getStatedValue() instanceof Throwable
            ...
            : new KlabActorException(
                arg.getStatedValue() == null
    ...
    try {
      ...
      if (cls != null) {
        ...
        for (String key : constructor.getArguments().keySet()) {
          ...
          settings.put(
              key, arg instanceof KActorsValue ? evaluateInScope((KActorsValue) arg, scope) : arg);
        }
          ...
          for (String setting : settings.keySet()) {
            String methodName =
                setting.startsWith("set")
                    ? setting
                    : ("set"
                        + org.integratedmodelling.common.utils.Utils.Strings.capitalize(setting));
            Object argument = settings.get(setting);
            Method method = null;
            try {
              method =
                  cls.getMethod(methodName, argument == null ? Object.class : argument.getClass());
  ...
  private void executeCall(Call code, KActorsBehavior behavior, KActorsScope scope) {
    ...
    if (!"self".equals(receiverName)) {
      ...
      if (synchronize) {
        scope.warn(
            "External actor calls are being made within a synchronous scope: this should "
      ...
      if (library.getMethods().containsKey(messageName)) {
        ...
        if (method.getMethod() != null) {
          ...
          for (Object arg : code.getArguments().getUnnamedArguments()) {
            args.add(
                arg instanceof KActorsValue ? evaluateInScope((KActorsValue) arg, scope) : arg);
          }
          try {
            ((KActorsScope) scope)
                .setValueScope(
    ...
    if (scope.getValueScope() != null) {
      scope.setValueScope(
          invokeReactorMethod(scope.getValueScope(), messageName, code.getArguments(), scope));
      return;
    }
    ...
    if (executor == null) {
      Class<? extends ActionExecutor> actionClass =
        ...
        if (executor != null) {
          ...
          if (executor instanceof ActionExecutor.Actor) {
            ...
            if (code.getArguments().containsKey("tag")) {
              ...
              this.localActionExecutors.put(
                  ((ActionExecutor.Actor) executor).getName(), (ActionExecutor.Actor) executor);
            }
          }
   ...
   * @param arguments
   */
  public void evaluateAssertion(
      Object target, Assertion assertion, KActorsScope scope, Parameters<String> arguments) {
    ...
    if (arguments.containsKey("select")) {
      ...
      } else if (sel instanceof KActorsValue
          && ((KActorsValue) sel).getType() == ValueType.EXPRESSION) {
    ...
    if (target instanceof Observation observation && runtimeScope instanceof ContextScope) {
      ...
      for (Locator locator :
        ...
        if (compareExpression != null) {
          ...
          ok =
              args.get("self") == null && compareValue == null
    ...
    switch (kvalue.getType()) {
        ...
        if (obj instanceof Observable) {
          if (value instanceof Observation) {
            return scope
                ...
                .resolves(((Observation) value).getObservable(), (Observable) obj, null);
      ...
      case RANGE:
        return value instanceof Number
            && ((NumericRangeImpl) (kvalue.getStatedValue()))
                .contains(((Number) value).doubleValue());
      case REGEXP:
        break;
      case STRING:
      ...
      case TYPE:
        return value != null
            && (kvalue.getStatedValue().toString().equals(value.getClass().getCanonicalName())
                || kvalue
                    .getStatedValue()
                    .toString()
      ...
      case CONSTANT:
        return (value instanceof Enum
                && ((Enum<?>) value)
                    .name()
                    .toUpperCase()
                    .equals(kvalue.getStatedValue().toString()))
      ...
      case EMPTY:
        return value == null
            ...
            || (value instanceof Observation
                && ((Observation) value).getObservable().is(SemanticType.NOTHING));
      case OBJECT:
        break;
      default:
        break;
   ...
   * @param scope
   */
  public Object invokeReactorMethod(
      Object reactor, String methodName, Parameters<String> arguments, KActorsScope scope) {
  ...
  /**
   * Return the boolean value of the passed object. In k.Actors, anything that isn't a null, false,
   * empty string or zero is true.
   *
   * @param ret
   * @return
   */
  private static boolean asBooleanValue(Object ret) {
    if (ret == null
        || (ret instanceof String && ((String) ret).trim().isEmpty())
