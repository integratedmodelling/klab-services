...
public class DoubleStorage implements Storage {
   ...
   * @return
   */
  public Data.DoubleFiller buffer(Geometry bufferGeometry, Data.FillCurve fillCurve) {
    return null;
  }
  ...
  /**
   * A quick-access buffer that simply addresses a dimension using a long. Obtained through {@link
   * #getSliceBuffer(Locator)}.
   */
  public class DirectSliceBuffer {
    ...
    /**
     * <code>set</code> will set the data but not insert the value in the histogram. Use to modify
     * the values. It will invalidate the histogram when used.
     *
     * @param value
     * @param position
     */
    ...
    /**
     * <code>add</code> should be used for successive inserts that cover the entire value space. It
     * will insert the value in the histogram if one is configured.
     *
     * @param value
     * @param position
     */
  ...
  /**
   * Using the slice buffer provides the quickest access with little code impact. Using the buffer
   * is alternative to the standard get/set and should be done only when one dimension is scanned at
   * the time and the dimension size is high.
   *
   * @param locator
   * @return
   */
  public DirectSliceBuffer getSliceBuffer(Locator locator) {
    ...
    return (time == null || time.is(Time.Type.INITIALIZATION))
        ? getOrCreateBuffer(0l, 0l)
        : getOrCreateBuffer(
            time.getFocus() == null
                ? time.getStart().getMilliseconds()
                : time.getFocus().getMilliseconds(),
            time.getStart().getMilliseconds());
  }
  ...
  /**
   * Map the passed operator within as many threads as specified by the level of parallelism defined
   * in the constructor. The operator returns the value for the passed long offset, which translates
   * the {@link Locator} produced by iterating a {@link Scale}.
   *
