...
public class EnvelopeImpl implements Envelope {
   ...
   * the readable unit string for it. For defaults and used when observing raw datasets. Reasoning:
   * give the envelope enough resolution to show enough square pixels to occupy half the screen by
   * computing the grid size against the m/pixel for the zoom level.
   *
   * <p>Meters per pixel at zoom level z are equal to 156412/(2^z).
   *
  ...
  public boolean intersects(Envelope envelope) {
    return this.envelope.intersects(
        (org.locationtech.jts.geom.Envelope) ((EnvelopeImpl) envelope).envelope);
  }
  ...
  public Envelope transform(Projection projection, boolean lenient) {
    ...
    throw new KlabUnimplementedException("Envelope::transform");
    //		return null;
  }
  ...
  public static EnvelopeImpl create(
    ...
    EnvelopeImpl ret = new EnvelopeImpl();
    ret.envelope = new ReferencedEnvelope((CoordinateReferenceSystem) envelope);
    ret.projection = projection;
    return ret;
  }
  ...
  public static EnvelopeImpl create(
      double minX, double maxX, double minY, double maxY, Projection projection) {
    return create(
  ...
  public boolean overlaps(Envelope other) {
    try {
      return this.envelope.intersects(
          ((EnvelopeImpl) other).getJTSEnvelope().toBounds(this.projection.getCRS()));
