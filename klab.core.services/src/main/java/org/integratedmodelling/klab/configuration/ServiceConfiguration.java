...
/**
 * TODO Rename and factor away all the duplicated methods with the Configuration available in the
 * api package
 *
 * @author Ferd
 * @version $Id: $Id
...
public enum ServiceConfiguration {
  ...
  static {
    ...
    Klab.INSTANCE.setConfiguration(
        new Klab.Configuration() {
          ...
          private Projection latlonProjection =
              new ProjectionImpl(ProjectionImpl.DEFAULT_PROJECTION_CODE);
          ...
          public Concept getNonSemanticConcept(SemanticType semanticType) {
            ...
            switch (semanticType) {
              ...
              case RELATIONSHIP -> {
                ret.setUrn("relationship");
                ret.setReferenceName("each_relationship");
                ret.setType(
                    EnumSet.of(
  ...
  //                ret.add(createVerbPrototype(namespacePrefix, clss.getAnnotation(Verb.class),
  // clss,
  //                null));
  //            } else if (clss.isAnnotationPresent(KlabAnnotation.class)) {
  //                ret.add(createAnnotationPrototype(namespacePrefix,
  // clss.getAnnotation(KlabAnnotation
  //                .class)
  //                        , clss,
  //                        null));
  //            }
  ...
  // method.isAnnotationPresent(KlabFunction
  //            .class)) {
  //                ret.add(createContextualizerPrototype(namespacePrefix,
  ...
  //                ret.add(createAnnotationPrototype(namespacePrefix,
  // cls.getAnnotation(KlabAnnotation
  //                .class),
  //                        cls,
  //                        method));
  ...
  //    }
  //
  //    public ServiceInfo createVerbPrototype(String namespacePrefix, Verb annotation, Class<?>
  // clss,
  //                                           Method method) {
  //        // TODO
  //        return null;
  //    }
  //
  //    public ServiceInfo createContextualizerPrototype(String namespacePrefix, KlabFunction
  // annotation,
  //                                                     Class<?> clss,
  //                                                     Method method) {
  //
  ...
  //        ret.setGeometry(annotation.geometry().isEmpty() ? null :
  // Geometry.create(annotation.geometry
  //        ()));
  ...
  //    }
  //
  //    public ServiceInfo createAnnotationPrototype(String namespacePrefix, KlabAnnotation
  // annotation,
  //                                                 Class<?> clss,
  //                                                 Method method) {
  //
  ...
  //
  //    /**
  //     * Check for updates and load all registered components; return the set of packages to scan
  // from
  //     them.
  //     * <p>
  //     * TODO!
  ...
  // componentUpdateManager.getLastPluginRelease
  //                (plugin.id);
  //                String lastVersion = lastRelease.version;
  //                String installedVersion = componentManager.getPlugin(plugin.id).getDescriptor()
  //                .getVersion();
  //                Logging.INSTANCE.debug("Update plugin '{}' from version {} to version {}",
  // plugin.id,
  //                        installedVersion,
  //                        lastVersion);
  ...
  //                (plugin.id);
  //                String lastVersion = lastRelease.version;
  //                Logging.INSTANCE.debug("Install plugin '{}' with version {}", plugin.id,
  // lastVersion);
  //                boolean installed = componentUpdateManager.installPlugin(plugin.id,
  // lastVersion);
  //                if (installed) {
   ...
   *
   * <p>TODO use plug-in manifest to declare packages to scan. Moved to ClassGraph to allow
   * development with Java 21 before Spring Boot 3.2.0 is released
   *
   * @param packageId
   * @return all annotations found with the corresponding class
   */
  public List<Pair<Annotation, Class<?>>> scanPackage(
      String packageId,
    ...
    try (ScanResult scanResult =
        new ClassGraph().enableAnnotationInfo().acceptPackages(packageId).scan()) {
  ...
  public File getDefaultExportDirectory() {
    File ret =
        new File(
            getProperties().getProperty(KLAB_EXPORT_PATH, dataPath + File.separator + "export"));
    ret.mkdirs();
    return ret;
  }
  ...
  /**
   * Retrieve the file with the passed relative path, creating subdirs as required. Must use forward
   * slash as separator. File is created empty if not existing.
   *
  ...
  /**
   * Same as {@link #getFile(String)} but if the file does not exist, create it with the content
  ...
  public File getTemporaryDataDirectory() {
    return new File(
        getProperties()
            .getProperty(
                KLAB_TEMPORARY_DATA_DIRECTORY, System.getProperty("java" + ".io" + ".tmpdir")));
  }
  ...
  public int getMaxWaitTime() {
    return Integer.parseInt(
        getProperty(ServiceConfiguration.KLAB_ACCEPTED_WAIT_TIME_SECONDS, "10"));
  }

  public String getProductsBranch() {
    return getProperty(
        ServiceConfiguration.KLAB_PRODUCTS_BRANCH, ServiceConfiguration.DEFAULT_PRODUCTS_BRANCH);
  }

  public URL getLocalComponentRepositoryURL(String servicePath) {
    try {
      return getDataPath(servicePath == null ? "components" : (servicePath + "/" + "components"))
          .toURI()
          .toURL();
   ...
   * found, throw a {@link KlabServiceAccessException}.
   *
   * @param <T>
   * @param serviceClass
  ...
  public <T extends Service> T getService(Class<T> serviceClass, Object... parameters)
      throws KlabServiceAccessException {
  ...
  /**
   * Return all registered and available services of the passed class, potentially filtering through
   * the passed parameters. If none available, return an empty collection. If multiple services are
   * available, they should be sorted with the "best" service on top.
   *
   * @param <T>
   * @param serviceClass
   * @param parameters any POD or Comparable
  ...
  public <T extends Service> Collection<T> getServices(
      Class<T> serviceClass, Object... parameters) {
  ...
  public void registerService(
      Service service, Class<? extends Service> keyClass, Object... parameters) {
   ...
   * @return
   */
  public <S extends KlabService, T> Collection<T> broadcastRequest(
      Function<S, T> request, Scope scope, Class<S> serviceClass) {
   ...
   */
  public <S extends KlabService, T> Collection<T> mergeServiceResults(
      Class<S> serviceClass,
      Supplier<Collection<T>> retriever,
      Function<Collection<Collection<T>>, Collection<T>> merger,
      Class<? extends T> individualResponseType,
      Channel monitor) {
