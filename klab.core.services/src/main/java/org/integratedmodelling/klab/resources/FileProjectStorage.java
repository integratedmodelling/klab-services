...
public class FileProjectStorage implements ProjectStorage {
  ...
  public RepositoryState getRepositoryState() {
    ...
    if (isTracked()) {
      ...
      ret.getNotifications()
          .add(Notification.info("Project " + projectName + " is not shared on a " + "repository"));
    }
    return ret;
  }
  ...
  //
  //        if (isTracked() && project instanceof ProjectImpl pimpl && project.getRepository()
  // instanceof
  ...
  // !remote.getURIs().isEmpty()) {
  //                                projectMetadata.setRepositoryUrl(new
  // URI(remote.getURIs().getFirst()
  //                                .toASCIIString()).toURL());
  //                            }
  //                        }
  //                    }
  ...
  //
  //                                var filePath =
  // ProjectStorage.getRelativeFilePath(resource.getUrn(),
  //                                        resourceType, "/");
  //
              ...
              (changedFile, action) -> {
                ...
                if (operation != null && type != null && !locked.get()) {
                  try {
                    notifier.apply(
                        projectName,
                        List.of(Triple.of(type, operation, changedFile.toURI().toURL())));
                  } catch (MalformedURLException e) {
                    throw new KlabIOException(e);
                  }
                }
  ...
  protected void collectResources(
      String extension, String sourceFolder, boolean recurse, List<URL> resultUrls) {

    File root =
        new File(this.rootFolder + File.separator + sourceFolder.replaceAll("/", File.separator));
    if (root.isDirectory()) {
