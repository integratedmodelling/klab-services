...
public final class WorldviewDeclarationSupport {
  ...
  public static void compile(OWL owl, Ontology ontology, Concept owner,
      KimConceptStatement statement, Function<KimConcept, Concept> resolve) {
    ...
    add(restrictions, "odo:affects", statement.getQualitiesAffected(), SemanticType.QUALITY, resolve);
    add(restrictions, "odo:creates", statement.getObservablesCreated(), SemanticType.OBSERVABLE, resolve);
    add(restrictions, "odo:requiresIdentity", statement.getRequiredIdentities(), SemanticType.IDENTITY, resolve);
    add(restrictions, "odo:requiresRealm", statement.getRequiredRealms(), SemanticType.REALM, resolve);
    add(restrictions, "odo:requiresExtent", statement.getRequiredExtents(), SemanticType.EXTENT, resolve);
    add(restrictions, "odo:requiresAttribute", statement.getRequiredAttributes(), SemanticType.ATTRIBUTE, resolve);
    add(restrictions, "odo:impliesObservable", statement.getImpliedObservables(), SemanticType.OBSERVABLE, resolve);
    add(restrictions, "odo:emergesFrom", statement.getEmergenceTriggers(), SemanticType.OBSERVABLE, resolve);
    for (var description : statement.getObservablesDescribed()) {
      String property = switch (description.getSecond()) {
      ...
      add(restrictions, property, List.of(description.getFirst()), SemanticType.QUALITY, resolve);
    }
    for (var link : statement.getSubjectsLinked()) {
      ...
      if (!bounds.accepts(owner, SemanticRole.RELATIONSHIP_SOURCE, source)
        ...
        throw new KlabValidationException("links endpoints must specialize the inherited links fillers");
    ...
    var applicables = statement.getAppliesTo().stream()
        .map(value -> require(resolve, value.getTarget(), SemanticType.OBSERVABLE)).toList();
    ...
    for (var applicable : applicables)
      if (!inheritedDomain.applicableTo(owner, applicable))
        throw new KlabValidationException("applies to must specialize every inherited applicability domain");
    for (var entry : restrictions.entrySet()) {
      var property = owl.getProperty(entry.getKey());
      if (property == null) throw new KlabValidationException("Missing ontology property " + entry.getKey());
  ...
  private static void add(Map<String, List<Concept>> restrictions, String property,
      List<KimConcept> operands, SemanticType expected, Function<KimConcept, Concept> resolve) {
    if (!operands.isEmpty()) {
  ...
  private static Concept require(Function<KimConcept, Concept> resolve, KimConcept syntax,
      SemanticType expected) {
    var concept = syntax == null ? null : resolve.apply(syntax);
    if (concept == null || concept.is(SemanticType.NOTHING) || !concept.is(expected))
      throw new KlabValidationException("Expected a resolved " + expected + " target: "
