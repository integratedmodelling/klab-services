...
//    boolean fluidUnits;
	...
	public static ObservableBuilder getBuilder(Concept concept, Channel monitor, OWL owl) {
		return new ObservableBuilder(concept, monitor, owl);
	}

	public static ObservableBuilder getBuilder(Observable observable, Channel monitor, OWL owl) {
		return new ObservableBuilder(observable, monitor, owl);
	}

	public ObservableBuilder(Concept main, Ontology ontology, Channel monitor, OWL owl) {
		this.owl = owl;
		this.main = main;
	...
	public ObservableBuilder(Concept main, Channel monitor, OWL owl) {
		this.owl = owl;
		this.main = main;
	 ...
	 * @param observable
	 */
	public ObservableBuilder(Observable observable, Channel monitor, OWL owl) {
		this.owl = owl;
	...
	KimConceptImpl getDeclaration(Concept concept) {
		return (KimConceptImpl) owl.reasoner().resolveConcept(concept.getUrn());
	}
	...
	public Observable.Builder as(UnarySemanticOperator type, Concept... participants) throws KlabValidationException {
		...
		if (incarnatedAbstractObservable != null) {
			incarnatedAbstractObservable = incarnatedAbstractObservable.builder().as(type, participants)
					.buildObservable();
		}
		...
		if (argument != null) {
			...
			try {
				...
				case VALUE:
				case MONETARY_VALUE:
					reset(owl.makeValue(argument, this.comparison, true, type == UnarySemanticOperator.MONETARY_VALUE),
							type);
					break;
				case MAGNITUDE:
	...
	public Observable.Builder without(SemanticRole... roles) {
		...
		ObservableBuilder ret = new ObservableBuilder(owl.reasoner().declareConcept(newDeclaration), monitor, owl);
	...
	private boolean resolveMain() {
		...
		if (ontology == null) {
			if (mainId != null) {
				...
				if (ontology == null) {
					errors.add(new KValidationException(
	...
	public Concept buildConcept() throws KlabValidationException {
		...
		if (roles != null && roles.size() > 0) {
			for (Concept role : roles) {
				if (owl.reasoner().roles(main).contains(role)) {
					monitor.error("concept " + main.displayName() + " already has role " + role.displayName(),
							declaration);
				}
		...
		if (monitor != null && !owl.reasoner().satisfiable(ret)) {
