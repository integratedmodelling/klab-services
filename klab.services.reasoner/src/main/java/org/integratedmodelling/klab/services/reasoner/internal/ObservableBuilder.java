...
public class ObservableBuilder implements Observable.Builder {
    ...
    @Override
    public Observable.Builder withDeclaration(KimConcept declaration) {
    ...
    public Observable.Builder as(UnarySemanticOperator type, Concept... participants) throws KlabValidationException {
        ...
        if (argument != null) {
            ...
            try {
                ...
                case VALUE:
                case MONETARY_VALUE:
                    reset(owl.makeValue(argument, this.comparison, true, type == UnarySemanticOperator.MONETARY_VALUE), type);
                    break;
                case MAGNITUDE:
    ...
    public Concept buildConcept() throws KlabValidationException {
        ...
        if (causant != null) {
            ...
            if (other != null && !owl.reasoner().compatible(causant, other)) {
                scope.error("cannot set the causant type of " + main.displayName() + " to " + causant.displayName()
        ...
        if (adjacent != null) {
            ...
            if (other != null && !owl.reasoner().compatible(adjacent, other)) {
                scope.error("cannot set the adjacent type of " + main.displayName() + " to " + adjacent.displayName()
        ...
        if (cooccurrent != null) {
            ...
            if (other != null && !owl.reasoner().compatible(cooccurrent, other)) {
                scope.error("cannot set the co-occurrent type of " + main.displayName() + " to " + cooccurrent.displayName()
        ...
        if (roles != null && roles.size() > 0) {
            for (Concept role : roles) {
                if (owl.reasoner().roles(main).contains(role)) {
                    scope.error("concept " + main.displayName() + " already has role " + role.displayName(), declaration);
                }
        ...
        if (acceptedRoles.size() > 0) {
            owl.restrictSome(ret, owl.getProperty(NS.HAS_ROLE_PROPERTY), LogicalConnector.UNION, acceptedRoles, ontology);
        }
        if (inherent != null) {
    ...
    private Ontology getTargetOntology() {
        return owl.getTargetOntology(ontology, main, traits, roles, inherent, context, caused, causant, compresent, goal,
    ...
    private String getCodeForm(Object o, boolean reference) {
        if (o == null) {
            ...
            return reference
                    ? owl.reasoner().declareConcept((KimConcept) o).getReferenceName()
                    : owl.reasoner().declareConcept((KimConcept) o).getName();
        } else if (o instanceof KimObservable) {
            return reference
                    ? owl.reasoner().declareObservable((KimObservable) o).getReferenceName()
