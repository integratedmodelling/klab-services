 ...
 * 
 * <p>
 * At this time using a semantic type is not required in the knowledge API (all access is through
 ...
 * 
 * <p>
 * Objects that are identified by semantic types are: Concepts, Properties, and Instances. Note that
 * Ontologies as such are not exposed by the KM interface, and the equivalence between the concept
 * space ID and an ontology is not to be relied upon. Even if in the current implementation a
 * concept space ID maps directly to an ontology URI, this may change and more sophisticated and
