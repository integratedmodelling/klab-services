...
public class OWL {
	...
	public void restrictSome(Concept target, Property property, Concept filler, Ontology ontology) {
		getTargetOntology(ontology, target, property, filler)
				.define(Collections.singleton(Axiom.SomeValuesFrom(target.getNamespace() + ":" + target.getName(),
	...
	public void restrictAll(Concept target, Property property, Concept filler, Ontology ontology) {
		getTargetOntology(ontology, target, property, filler)
				.define(Collections.singleton(Axiom.AllValuesFrom(target.getNamespace() + ":" + target.getName(),
						property.toString(), filler.getNamespace() + ":" + filler.getName())));
	}
	...
	/**
	 * type of <trait> makes a CLASS that incarnates the trait as a quality and
	 * whose values are the concrete children of the trait.
	 * 
	 * @param classified
	 * @param addDefinition
	 * @return
	...
	/**
	 * {@link IKnowledge#is(ISemantic)} will only check for direct subsumption. This
	 * one defaults to that when the reasoner is not active.
	 * 
	 * Annotation properties only use asserted methods.
	 * 
	 * TODO handle exceptions and/or default to is() when the ontology is
	 * inconsistent and we're not running in strict mode.
	 * 
	 * @param c1
	 * @param c2
	 * @return true
	...
	/**
	 * Synonym of getAllParents(), in Concept; defaults to asserted parents if
	 * reasoner is off. Will not return owl:Thing (which should never appear anyway)
	 * or owl:Nothing, even if the concept is inconsistent.
	 * 
	 * @param main
	...
	/**
	 * Synonym of getSubclasses, in Concepts; defaults to asserted children if
	 * reasoner is off. Will not return owl:Thing (which should never appear anyway)
	 * or owl:Nothing, which is a child of everything.
	 * 
	 * @param main
