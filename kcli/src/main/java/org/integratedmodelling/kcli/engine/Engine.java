...
public enum Engine implements Authentication {
    ...
    private Engine() {
        ...
        if (Utils.Network.isAlive("http://127.0.0.1:" + ResourcesService.DEFAULT_PORT + " /resources/actuator")) {
            EngineService.INSTANCE.setResources(new ResourcesClient("http://127.0.0.1:" + Reasoner.DEFAULT_PORT + " /resources"));
        } else {
            EngineService.INSTANCE
                    .setResources(new ResourcesProvider(this, EngineService.INSTANCE.newServiceScope(ResourcesService.class)));
        }

        if (Utils.Network.isAlive("http://127.0.0.1:" + Reasoner.DEFAULT_PORT + " /reasoner/actuator")) {
            EngineService.INSTANCE.setReasoner(new ReasonerClient("http://127.0.0.1:" + Reasoner.DEFAULT_PORT + " /reasoner"));
        } else {
            EngineService.INSTANCE.setReasoner(new ReasonerService(this, EngineService.INSTANCE.newServiceScope(Reasoner.class)));
        }
        ...
        if (Utils.Network.isAlive("http://127.0.0.1:" + Resolver.DEFAULT_PORT + " /resolver/actuator")) {
            EngineService.INSTANCE.setResolver(new ResolverClient("http://127.0.0.1:" + Resolver.DEFAULT_PORT + " /resolver"));
        } else {
            EngineService.INSTANCE.setResolver(new ResolverService(this, EngineService.INSTANCE.newServiceScope(Resolver.class)));
        }

        if (Utils.Network.isAlive("http://127.0.0.1:" + RuntimeService.DEFAULT_PORT + " /runtime/actuator")) {
            EngineService.INSTANCE.setRuntime(new RuntimeClient("http://127.0.0.1:" + RuntimeService.DEFAULT_PORT + " /runtime"));
        } else {
    ...
    /**
     * Return the specific service among the various available. The "local" keyword returns the
     * service installed as the default in the user scope. The "active" keyword should get the
     * currently active service. Anything else will look up the service using the discovery
     * mechanism.
     * 
     * @param <T>
     * @param name
