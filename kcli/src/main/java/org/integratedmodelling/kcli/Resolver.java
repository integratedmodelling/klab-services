    ...
    public static class Resolve extends FunctionalCommand {
        ...
        public void run() {
            ...
            if (ctx == null) {
                ...
                out.println(Ansi.AUTO.string("Resolution of @|yellow " + resolution.getResolvable() + "|@ terminated with @|"
                        + (resolution.getCoverage().isRelevant() ? "green " : "red ")
                        + NumberFormat.getPercentInstance().format(resolution.getCoverage().getCoverage()) + "|@ coverage"));
        ...
        public void run() {
            ...
            if (this.resolution instanceof Resolution) {
                ...
                if (dataflow.isEmpty()) {
                    ...
                    Utils.Files.writeStringToFile(json ? Utils.Json.printAsJson(dataflow) : resolver.encodeDataflow(dataflow),
                            output);
