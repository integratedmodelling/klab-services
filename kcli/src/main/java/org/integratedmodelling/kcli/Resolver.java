...
public class Resolver {
	...
	public static class Resolve extends FunctionalCommand {
		...
		public void run() {
			...
			if (ctx == null) {
				...
				out.println(Ansi.AUTO.string("Resolution of @|yellow " + resolution.getResolvable()
						+ "|@ terminated with @|" + (resolution.getCoverage().isRelevant() ? "green " : "red ")
						+ NumberFormat.getPercentInstance().format(resolution.getCoverage().getCoverage())
						+ "|@ coverage"));

				out.println(Ansi.AUTO.string(
						"@|" + (resolution.getCoverage().isRelevant() ? "green " : "red ") + resolution + "|@"));
	...
	@Command(name = "compile", mixinStandardHelpOptions = true, version = Version.CURRENT, description = {
			"Resolve some knowledge in the current context, compile a dataflow",
			"Prints or stores the JSON or the k.DL code", "Resolve any URN in the current context." }, subcommands = {})
	public static class Compile extends Resolve {
		...
		public void run() {
			...
			if (lastPushed instanceof Resolution) {
				...
				var dataflow = resolver.compile(this.knowledge, this.resolution, ctx);
				...
				if (dataflow.isEmpty()) {
					...
					Utils.Files.writeStringToFile(
							json ? Utils.Json.printAsJson(dataflow) : resolver.encodeDataflow(dataflow), output);
