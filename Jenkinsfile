        ...
        stage('Update services') {
            steps {
                sshagent(["bc3-im-services"]) {
                    sh "ssh -o StrictHostKeyChecking=no -l bc3 ${DOCKER_HOST} docker service update ${DOCKER_STACK}_${RESOURCE_SERVICE} --image ${REGISTRY}/${RESOURCES_CONTAINER}:${TAG} --with-registry-auth"
                }
            }
        }
    }
