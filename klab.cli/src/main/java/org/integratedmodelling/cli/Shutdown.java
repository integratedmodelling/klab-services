...
@CommandLine.Command(
    ...
    description = {"Shutdown the engine and all services started locally.", ""})
public class Shutdown implements Runnable {
  ...
  @CommandLine.Option(
      names = {"-e", "--exit"},
      description = "Wait for all local services to shutdown " + "and exit")
  boolean exit;
  ...
  public void run() {
    ...
    out.println(
        CommandLine.Help.Ansi.AUTO.string(
            "@|cyan Shutting down local services"
                + (exit ? (" and" + " exiting to OS") : "")
                + "|@"));
    ...
    if (exit) {
      ...
      if (ns > 0) {
        ...
        out.println(
            CommandLine.Help.Ansi.AUTO.string(
                "@|yellow Waiting for " + services.size() + " local services to exit|@"));
        ...
        while (true) {
          ...
          if (n == services.size()) {
            out.println(
                CommandLine.Help.Ansi.AUTO.string(
                    "@|green All local services have shut" + " down: exiting|@"));
            System.exit(0);
          }
