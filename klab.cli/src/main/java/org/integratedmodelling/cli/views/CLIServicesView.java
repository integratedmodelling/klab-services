...
@CommandLine.Command(name = "services", mixinStandardHelpOptions = true, description = {"List, select and control " +
                                                                                                "services.",
                                                                                        ...
                                                                                        "Service discovery is supported" +
                                                                                                ...
                                                                                                "credentials.", ""},
                     subcommands = {
                             org.integratedmodelling.cli.views.CLIServicesView.List.class,
...
public class CLIServicesView extends CLIView implements ServicesView {
    ...
    @CommandLine.Command(name = "list", mixinStandardHelpOptions = true, description = {"List the services available",
    ...
    static class List implements Runnable {
        ...
        @CommandLine.Option(names = {"-v", "--verbose"}, defaultValue = "false", description = {
                "Display status and capabilities from services"}, required = false)
        boolean verbose = false;

        @CommandLine.Option(names = {"-rs", "--reasoners"}, defaultValue = "false", description = {
                "List all reasoner services."}, required = false)
        boolean reasoners = false;

        @CommandLine.Option(names = {"-rv", "--resolvers"}, defaultValue = "false", description = {
                "List all resolver services."}, required = false)
        boolean resolvers = false;

        @CommandLine.Option(names = {"-rn", "--runtimes"}, defaultValue = "false", description = {
                "List all runtime services."}, required = false)
        boolean runtimes = false;

        @CommandLine.Option(names = {"-rr", "--resources"}, defaultValue = "false", description = {
                "List all resource services."}, required = false)
        boolean resources = false;

        @CommandLine.Option(names = {"-c", "--community"}, defaultValue = "false", description = {
    ...
    @CommandLine.Command(name = "connect", mixinStandardHelpOptions = true, description = {"Connect to an available " +
                                                                                                   "service",
                                                                                           "Makes the service available" +
                                                                                                   " for requests."})
    static class Connect implements Runnable {

        @CommandLine.Option(names = {"-d", "--default"}, defaultValue = "false", description = {
    ...
    public void engineStatusChanged(Engine.Status status) {
        System.out.println("CIAPA EL CAMBIO DIOCAN " + status);
