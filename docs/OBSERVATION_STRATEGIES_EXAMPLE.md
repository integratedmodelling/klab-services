...
| EX04 | `relationships.direct`, rank 0 | Resolve collective endpoints and observe their relationships | Implemented: named source/target inputs, connector output builders, participant graph edges, and individual acknowledgement in `between(...)` scopes | The model chooses pairs; Runtime persists observations in cohorts. Bonds have distinct unordered participant edges. |
