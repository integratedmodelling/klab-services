...
public class ResolverPorker {
    ...
    private ResolutionGraph resolve(Observation observation, ContextScope scope,
        ...
        for (var operation : observationStrategy.getOperations()) {
            ...
            switch (operation.getType()) {
                ...
                case RESOLVE -> {
                    ...
                    var contextualizedScope = contextualizeScope(scope, operation.getObservable(),
                            scaleToCover
                            , graph);
                    ...
                    var observableResolution = resolve(operation.getObservable(),
                            contextualizedScope.getSecond(), ret,
                ...
                case OBSERVE -> {
                    ...
                    for (Model model : queryModels(operation.getObservable(),
                            contextualizedScope.getFirst(), contextualizedScope.getSecond())) {
    ...
    private Pair<ContextScope, Scale> contextualizeScope(ContextScope originalScope, Observable observable,
        ...
        if (observable.isCollective()) {
            ...
            if (context == null) {
                scope.error("Cannot resolve a dependent without a context substantial observation: " + observable.getUrn());
                return null;
            }
    ...
    private ResolutionGraph resolve(Observable observable, Scale scaleToCover, ResolutionGraph graph,
        ...
        var observation = requireObservation(observable, contextualizedScope.getFirst(),
     ...
     */
    private Observation requireObservation(Observable observable, ContextScope scope, Geometry geometry) {
        ...
        /* TODO this should also happen if the inherency is incompatible with the semantics for dependent
            observables */
        if (ret.isEmpty()) {
            scope.error("Cannot instantiate observation of " + observable.getUrn() + " in context " + scope.getId());
            return null;
        }
