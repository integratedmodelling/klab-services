...
public class ResolutionGraph {
     ...
     * {@link ResolutionEdge.Type} enum. These should be treated separately when building the
     * dataflow: all DIRECT should be done first by creating new actuators, then any FILTERs added,
     * and last the DEFERRED strategies should cause the runtime to invoke the <em>same</em>
     * resolver again. The "resolving" nodes for each strategy can be accessed from a node using the
     * {@link #getResolving(ResolutionEdge.Type)} method.
     * 
     * @author Ferd
     *
     */
    public class Resolution {
         ...
         */
        public Resolution resolve(Resolution child, Resolution.Type resolutionType) {
         ...
         * {@link #resolve(Resolution, ResolutionGraph.ResolutionEdge.Type)} on a node that should
         * be accepted in the graph, which shouldn't really happen.
         */
        public void accept() {
