...
/**
 * The basic k.LAB actor has a state hash and can run one or more k.Actors behaviors as a response
 * to a RunBehavior message. The behaviors run asynchronously in a separate VM (one per agent,
 * running all the behaviors that were sent in separate threads). Each run uses a run scope that
 * contains the actor ref and the Scope that the agent was created in.
 * <p>
 * Implementations dedicated to running single behaviors (script, application and test case) will
 * call {@link #run(KActorsBehavior, Scope)} directly upon initialization.
 * 
...
public abstract class KAgent implements ReActor {
    ...
    public static class KAgentRef implements Ref {
        ...
        public <T extends Serializable, R extends Serializable> R ask(T message, Class<? extends R> responseClass,
            ...
            this.ref.ask(message, replyClass, timeout, "request_" + nextId.incrementAndGet()).whenComplete((reply, failure) -> {
    ...
    private void runBehavior(ReActorContext rctx, RunBehavior message) {
        KActorsBehavior behavior = message.getScope().getService(ResourceProvider.class).resolveBehavior(message.getBehavior(),
                message.getScope());
        if (behavior != null) {
