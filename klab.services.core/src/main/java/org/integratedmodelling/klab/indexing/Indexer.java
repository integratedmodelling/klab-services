...
public class Indexer {
    ...
    public Indexer(Scope scope) {
        ...
        try {
             ...
             */
            nrtReopenThread = new ControlledRealTimeReopenThread<IndexSearcher>(writer, searcherManager,
                    1.0, 0.1);
            nrtReopenThread.setName("NRT Reopen Thread");
            nrtReopenThread.setPriority(Math.min(Thread.currentThread().getPriority() + 2,
                    Thread.MAX_PRIORITY));
        ...
        try {
            ...
            for (String key : resource.getMetadata().keySet()) {
                if (Metadata.DC_DESCRIPTION.equals(key)) {
                    document.add(new TextField("description", resource.getMetadata().get(key).toString(),
                            Store.YES));
                } else {
    ...
    public SemanticMatch index(KlabStatement object) {
        ...
        if (ret != null) {
            ...
            try {
                ...
                document.add(new IntPoint("ctype",
                        SemanticType.fundamentalType(ret.getConceptType()).ordinal()));
                document.add(new IntPoint("mtype", ret.getMatchType().ordinal()));
                document.add(new IntPoint("abstract", ret.isAbstract() ? 1 : 0));
                // ..store them
                document.add(new StoredField("vctype",
                        SemanticType.fundamentalType(ret.getConceptType()).ordinal()));
     ...
     * interested in. We will return matches in order of request, incrementally filtering any duplicates.
     * <p>
     * The arguments may be values of the {@link SemanticRole} enum (which specify operators, types or roles)
     * or {@link SemanticType}s for concept categories, or collections thereof. The may also be filters for
     * properties to apply to concepts.
     *
     * @param term
     * @return
     */
