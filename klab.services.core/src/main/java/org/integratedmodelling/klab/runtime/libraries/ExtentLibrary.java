...
public class ExtentLibrary {
    ...
    @KlabFunction(name = "space", description = "Create spatial extents of all supported types", type = Type.SPATIALEXTENT, version = Version.CURRENT, parameters = {
            @Argument(name = "shape", type = Type.TEXT, description = "A geometric shape in WKT or WKB form"),
            @Argument(name = "grid", type = {Type.QUANTITY, Type.TEXT}, description = "Grid resolution", optional = true)})
    public static Space space(ServiceCall call, Scope scope) {
        ...
        // throw new KlabIllegalArgumentException("urn " + urn + " does not resolve to a spatial
        // object");
        // }
        ...
        // ret = org.integratedmodelling.klab.components.geospace.extents.Space.constraint(shape,
        // true);
        // }
    ...
    @KlabFunction(name = "time", description = "Create temporal extents of all supported types", type = Type.TEMPORALEXTENT, version = Version.CURRENT, parameters = {
            @Argument(name = "grid", type = {Type.QUANTITY,
                    Type.TEMPORAL_RESOLUTION}, description = "A grid resolution with a temporal unit", optional = true),
            @Argument(name = "year", type = Type.NUMBER, description = "A grid resolution with a temporal unit", optional = true),
            @Argument(name = "start", type = Type.DATETIME, description = "A grid resolution with a temporal unit", optional = true),
            @Argument(name = "end", type = Type.DATETIME, description = "A grid resolution with a temporal unit", optional = true),
            @Argument(name = "step", type = Type.DATETIME, description = "A grid resolution with a temporal unit", optional = true),
            @Argument(name = "realtime", type = Type.BOOLEAN, description = "A grid resolution with a temporal unit", optional = true),
            @Argument(name = "generic", type = Type.BOOLEAN, description = "A grid resolution with a temporal unit", optional = true),
            @Argument(name = "focus", type = Type.DATETIME, description = "A grid resolution with a temporal unit", optional = true)})
    public static Time time(ServiceCall call, Scope scope) {
        ...
        if (parameters.contains("focus")) {
            ...
            if (parameters.get("focus") instanceof Quantity) {
                ...
                try {
                    Time.Resolution.Type focus = Time.Resolution.Type.parse(parameters.get("focus").toString());
        ...
        if (parameters.contains("step")) {
            ...
            if (parameters.get("step") instanceof Quantity) {
                ...
                throw new KValidationException("wrong specification of step in time function: expecting number with units. "
    ...
    @KlabFunction(name = "geometry", description = "Create a scale based on a geometry or URN specification", type = Type.GEOMETRY, version = Version.CURRENT, parameters = {
            @Argument(name = "value", type = {Type.TEXT,
                    Type.URN}, description = "A geometry string or a resource URN, whose geometry is used")})
