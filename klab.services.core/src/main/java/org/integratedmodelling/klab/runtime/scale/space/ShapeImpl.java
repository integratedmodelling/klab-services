...
public class ShapeImpl extends SpaceImpl implements Shape {
	...
	public boolean containsCoordinates(double x, double y) {
		checkPreparedShape();
		return preparedShape == null ? geometry.contains(SpaceImpl.gFactory.createPoint(new Coordinate(x, y)))
...
//	@Override
//	public Space merge(ITopologicallyComparable<?> other, LogicalConnector how, MergingOption... options) {
//
...
//	}
	...
	public Shape getSimplified(IQuantity resolution) {
		...
		// in m
		double simplifyFactor = Services.INSTANCE.getService(UnitService.class).meters()
				.convert(resolution.getValue(), unit).doubleValue();
