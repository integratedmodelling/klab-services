...
public class KActorsVM implements VM {
    ...
    public static Object evaluateInScope(KActorsValue arg, KActorsScope scope) {
        ...
        case EXPRESSION:
            ...
            try {
                ...
                // Parameters.create(scope.getSymbols(identity), "metadata", scope.getMetadata(),
                // "self", identity));
            } catch (Throwable t) {
                scope.error(t);
                return null;
    ...
    public void evaluateAssertion(Object target, Assertion assertion, KActorsScope scope, Parameters<String> arguments) {
        ...
        if (selector != null) {
            ...
            // if (selectDescriptor.isScalar(input) && runtimeScope.getArtifact(input, IState.class)
            // != null) {
            // IState state = runtimeScope.getArtifact(input, IState.class);
            // if (state != null) {
            // states.put(state.getObservable().getName(), state);
    ...
    public boolean matches(KActorsValue kvalue, Object value, KActorsScope scope) {
        ...
        case TYPE:
            return value != null && (kvalue.getStatedValue().get(String.class).equals(value.getClass().getCanonicalName())
                    || kvalue.getStatedValue().get(String.class).equals(Path.getLast(value.getClass().getCanonicalName(), '.')));
        case URN:
            break;
        case ERROR:
