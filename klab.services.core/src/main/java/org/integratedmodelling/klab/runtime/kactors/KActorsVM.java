 ...
 * gracefully or warn and move on.
 * 
...
//    private Ref parentActor = null;
	 ...
	 * @param behavior
	 */
	public void runBehavior(KActorsBehavior behavior, Parameters<String> arguments, Scope scope) {
					...
					if (behavior.getType() == KActorsBehavior.Type.UNITTEST) {
						for (KActorsAction action : getActions(behavior, "@test")) {
							...
							if (desc.get("enabled", Boolean.TRUE) && !desc.get("disabled", Boolean.FALSE)) {
								...
								testScope.getMonitor().info(behavior.getName() + ": running test " + action.getName());
						...
						scope.getMonitor().info(behavior.getName() + ": done running tests");
					}
	...
	protected void run(KActorsAction action, KActorsBehavior behavior, KActorsScope scope) {
		...
		try {
			...
			if (scope.getSender() != null) {
				scope.getSender().tell(new Fire(scope.getListenerId(), t, scope.getAppId(), scope.getSemaphore(),
...
//            }
	...
	private void executeAssert(Assert code, KActorsBehavior behavior, KActorsScope scope) {
		...
		for (Assertion assertion : code.getAssertions()) {
			...
			if (assertion.getValue() != null || assertion.getExpression() != null) {
				...
				TestBehavior.evaluateAssertion(
						scope.getMatchValue() == null ? scope.getValueScope() : scope.getMatchValue(), assertion, scope,
						code.getArguments());
			}
		}
	}
	...
	private boolean executeFire(FireValue code, KActorsScope scope) {
		...
		if (((KActorsScope) scope).getSender() != null) {
			 ...
			 */
			// parentActor
			// .tell(new ComponentFire(receiver.path().name(),
			// code.getValue().evaluate(scope,
			...
			if (library.methods.containsKey(messageName)) {
				...
				if (method.method != null) {
					...
					if (scope.getValueScope() != null) {
						 ...
						 */
						boolean ok = method.method.getParameterCount() > 0 && scope.getValueScope().getClass()
								.isAssignableFrom(method.method.getParameters()[0].getType());
					...
					try {
						((KActorsScope) scope).setValueScope(
								method.method.invoke(nativeLibraryInstances.get(library.name), args.toArray()));
						return;
					} catch (Throwable e) {
