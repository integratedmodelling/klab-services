...
public class KActorsVM implements VM {
    ...
    // /*
    // * This is the parent that generated us through a 'new' instruction, if any. FIXME should be
    // in
    // * the scope
    // */
    // @Deprecated
...
//                                }
                    ...
                    if (rootView && scope.getViewScope().getLayout() != null) {
                        ...
                        scope.getMonitor().send(Message.MessageClass.UserInterface, Message.Type.SetupInterface,
...
//                    }
    ...
    private void executeInstantiation(Instantiation code, KActorsBehavior behavior, KActorsScope scope) {
		...
		if (code.getArguments() != null) {
			 ...
			 * main()
			 */
			KActorsBehavior childBehavior = scope.getMainScope().getService(ResourceProvider.class).resolveBehavior(code.getBehavior(), scope.getMainScope());
		...
		KActorsBehavior actorBehavior = scope.getMainScope().getService(ResourceProvider.class).resolveBehavior(code.getBehavior(), scope.getMainScope());
    ...
    private void executeAssignment(Assignment code, KActorsScope scope) {
        if (code.getRecipient() != null) {
            if ("self".equals(code.getRecipient())) {
                // NO cazzo - send a message diocan
                scope.getIdentity().getState().put(code.getVariable(), ((KActorsValue) code.getValue()).evaluate(scope, false));
            } else {
...
//                    if (scope.getIdentity() instanceof IRuntimeIdentity) {
            ...
            if (synchronize) {
                scope.getMonitor().warn("External actor calls are being made within a synchronous scope: this should "
