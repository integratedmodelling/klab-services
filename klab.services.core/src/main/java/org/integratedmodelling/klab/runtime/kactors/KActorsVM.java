 ...
 * KlabActor. Each actor should own a VM but components will be routines, not
 * actors, thereby saving the whole mess of forward IDs. The VM must be fully
 * reentrant w.r.t. behaviors, actions and state.
 * <p>
 * VMs should be usable without an actor as long as no actor-specific calls are
 * made. These should be aware that the actor can be null and terminate
 * gracefully or warn and move on. Communication with the actor is handled by
 * scope.send(), which in agent-enabled scopes should filter VM.AgentMessage
 * messages and tell() them to the agent Ref.
 * <p>
...
public class KActorsVM implements VM {
	...
	/**
	 * Top-level. TODO pass arguments and whatever else needs to be defined in the
	 * root scope.
	 * 
	 * @param behavior
	 */
	...
	public void run(KActorsBehavior behavior, Parameters<String> arguments, Scope scope) {
		runBehavior(behavior, arguments, new KActorsScope(scope, behavior));
	}
				...
				try {
					...
					for (KActorsAction action : getActions(behavior, "init", "@init")) {
						...
						if (initScope.isInterrupted() || initScope.hasErrors()) {
							/*
							 * TODO if testing and init fails, the test is skipped. If not testing and init
							 * fails, the rest of the behavior is not loaded.
							 */
...
//						scope.getMainScope().send(Message.MessageClass.UserInterface, Message.Type.SetupInterface,
...
//                    if (scope.getIdentity() instanceof Session) {
//					if (KActorsVM.this.appId != null && (behavior.getType() == KActorsBehavior.Type.SCRIPT
//							|| behavior.getType() == KActorsBehavior.Type.UNITTEST)) {
					/*
	...
	protected void run(KActorsAction action, KActorsBehavior behavior, KActorsScope scope) {
		...
		try {
			...
			if (scope.getSender() != null) {
				...
				/*
				 * Fart in space: nothing is listening from the behavior being executed. TODO -
				 * an actor firing with no action listening and no parent should just send to
				 * either the user actor or (maybe) its parent identity? TODO - the outer group
				 * may be listening.
				 */
			}
		}
...
//			scope.getIdentity().setView(new ViewImpl(scope.getViewScope().getLayout()));
			scope.send(IMessage.MessageClass.UserInterface,
...
//		executeCall(new KActorsActionCall(code), behavior, scope);
	...
	private void executeAssert(Assert code, KActorsBehavior behavior, KActorsScope scope) {
		...
		for (Assertion assertion : code.getAssertions()) {
			...
			if (assertion.getValue() != null || assertion.getExpression() != null) {
				...
				evaluateAssertion(scope.getMatchValue() == null ? scope.getValueScope() : scope.getMatchValue(),
	...
	/**
	 * If the call is a known function, call it and leave the value in the scope.
	 * Otherwise check if it's a method of the valueScope receiver if we have it.
	 * 
	 * @param call
	 * @param fscope
	 */
	...
	/**
	 * A call sequence is a one or more calls to be executed in sequence. The last
	 * call is a standard message call which will either fire or return according to
	 * the scope; the ones preceding it, if any, are necessarily functional and the
	 * return value of the first provides the execution context for the next.
	 * 
	 * @param calls
	 * @param scope
	 */
	...
	/**
	 * TODO add handling of test cases - all fires (including exceptions) should be
	 * intercepted
	 * 
	 * @param code
	 * @param scope\
...
//            ((KActorsScope) scope).getSender().tell(new Fire(scope.getListenerId(), evaluateInScope(code.getValue(), scope),
...
//                scope.getIdentity().getState().put(code.getVariable(), evaluateInScope(code.getValue(), scope)); // false
			} else {
	...
	public static Object evaluateInScope(KActorsValue arg, KActorsScope scope) {
		...
		case OBSERVABLE:
			...
			break;
		case ERROR:
			throw arg.getStatedValue() instanceof Throwable ? new KlabException((Throwable) arg.getStatedValue())
					: new KlabException(arg.getStatedValue() == null ? "Unspecified actor error from error value"
...
//                        Parameters.create(scope.getSymbols(identity), "metadata", scope.getMetadata(), "self", identity));
				...
				if (ret != null) {
					for (String setting : settings.keySet()) {
						...
						if (method != null) {
							...
							if (scope != null) {
								scope.warn("k.Actors: cannot find a " + methodName
										+ " method to invoke on constructed object");
							} else {
								Logging.INSTANCE.warn("k.Actors: cannot find a " + methodName
										+ " method to invoke on constructed object");
							}
						}
					}
	...
	private void executeCall(Call code, KActorsBehavior behavior, KActorsScope scope) {
		...
		if (!"self".equals(receiverName)) {
			...
			/*
			 * Check first if the recipient is a Java peer and in that case, use reflection
...
//                    if (scope.getIdentity() instanceof IRuntimeIdentity) {
		 ...
		 * context receiver in case we have one.
		 */
		for (Library library : libraries.values()) {
			if (library.getMethods().containsKey(messageName)) {
				...
				if (method.getMethod() != null) {
					...
					try {
						((KActorsScope) scope).setValueScope(method.getMethod()
								.invoke(nativeLibraryInstances.get(library.getName()), args.toArray()));
						return;
					} catch (Throwable e) {
		 ...
		 * 
		 * Now we're in the appropriate scope for synchronous execution if we have
		 * actions after fire.
		 */
		...
		if (actionCode != null || libraryActionCode != null) {
			 ...
			 */
			run(actionCode, behavior,
					((KActorsScope) scope)
							.matchFormalArguments(code, (actionCode == null ? libraryActionCode : actionCode))
		...
		if (executor instanceof WidgetActionExecutor) {
			...
			executor.run(((KActorsScope) scope).withNotifyId(notifyId).withSender(receiver));
		}

		/*
		 * if the scope was not synchronous, or there were no actions after a fire, this
		 * does nothing. TODO In case of errors causing no fire, though, it will wait
		 * forever, so there should be a way to break the wait.
		 */
		((KActorsScope) scope).waitForGreen(code.getFirstLine());
	...
	/**
	 * Used within KlabActor to compare a returned value with an expected one in a
	 * test scope. If we're not in test scope, send an exception to the monitor on
	 * lack of match.
	 * 
	 * @param scope
	 * @param comparison
...
//			}
	...
	public boolean matches(KActorsValue kvalue, Object value, KActorsScope scope) {
		...
		case TYPE:
			return value != null
					&& (kvalue.getStatedValue().get(String.class).equals(value.getClass().getCanonicalName())
		...
		case EMPTY:
			return value == null || (value instanceof Collection && ((Collection<?>) value).isEmpty())
					...
					|| (value instanceof Artifact && !(value instanceof ObservationGroup)
	...
	/**
	 * Invoke a method based on parameters from a call to a Java reactor inside the
	 * k.Actors code.
	 * 
	 * @param reactor
	 * @param arguments
	...
	public Object invokeReactorMethod(Object reactor, String methodName, Parameters<String> arguments,
		...
		if (method != null) {
			...
			if (method == null && kargs == null) {
				/*
				 * see if we have a method with the same args + a map of options and pass an
				 * empty option map if so.
				 */
			...
			if (scope != null) {
				...
				Logging.INSTANCE.warn("k.Actors: cannot find a '" + methodName
						+ "' method to invoke on object of class " + reactor.getClass().getCanonicalName());
			}
		}
	...
	/**
	 * Return the boolean value of the passed object. In k.Actors, anything that
	 * isn't a null, false, empty string or zero is true.
	 * 
	 * @param ret
	 * @return
	 */
