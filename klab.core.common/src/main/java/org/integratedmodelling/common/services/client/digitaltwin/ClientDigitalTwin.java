...
/**
 * Client-side digital twin, connected to the actual DT through the messages it gets from it (and nothing more
 * for now). Will NOT reconstruct the entire runtime-side DT and many functions will throw exxeptions at the
 * beginning. Later we may implement a lazy request-driven strategy to obtain the rest, but requesting entire
 * knowledge graphs should not be expected to work at client side for now. Same for state storage if it needs
 * to be used with any level of efficiency - the implementation should be limited to statistical info.
 */
public class ClientDigitalTwin implements DigitalTwin {
    ...
    /**
     * Main function that constructs the client-side KG structure. Not all elements in the remote KG will be
     * present, but those that are must be coherently linked.
     *
     * @param event
     */
    public void ingest(Message event) {
        // TODO
        System.out.println("INGESTING " + event);
    }
