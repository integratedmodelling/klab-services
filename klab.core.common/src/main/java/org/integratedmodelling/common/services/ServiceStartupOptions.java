...
public class ServiceStartupOptions implements StartupOptions {
  ...
  @Option(
      name = "-cert",
      usage =
          "certificate file (default: <dataDir>/"
  ...
  @Option(
      name = "-url",
      usage =
          "the host URL without port or context path. Communicated to clients so it must be right"
  ...
  public File getCertificateFile() {
    if (certificateFile == null) {
      certificateFile =
          new File(
                  Configuration.INSTANCE.getServiceDataPath()
                  + File.separator
                  + KlabCertificate.DEFAULT_SERVICE_CERTIFICATE_FILENAME);
    }
  ...
  public File getDataDirectory() {
    if (dataDir == null) {
      dataDir = Configuration.INSTANCE.getServiceDataPath();
    }
    return dataDir;
  }
  ...
  @Override
  public boolean isStartLocalBroker() {
    return startLocalBroker;
  }
