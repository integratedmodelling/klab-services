 ...
 * derived knowledge assets from documents upon loading, also handling versions.
 *
 * <p>TODO when used with locked services, should subscribe to resource events and reload any
...
public enum KnowledgeRepository {
  ...
  /**
   * Retrieve any knowledge asset registered by the service. This is meant to hold derived assets
   * such as models, concepts etc. as needed, and is normally used on the service side where
   * services can keep the assets they need.
   *
   * @param urn
   * @param assetClass
   * @param version
  ...
  /**
   * Load what necessary from the passed resource set and update any index. After this has returned,
   * any results in the resource set are returned directly in order of reference. This only loads
   * documents, which can be further processed by the passed functors to store assets they contain;
   * for components and the like, the extension mechanism must be extended. The functors determine
   * what gets returned and only operates on the resource set's results.
   *
   * @param resourceSet
   * @param scope
   * @return any resolved knowledge items pointed to by the resourceSet {@link
   *     ResourceSet#getResults()} method, or an empty list if the result list was empty or an error
   *     occurred.
   */
  public <T extends KlabAsset> List<T> ingest(
      ResourceSet resourceSet,
      Scope scope,
      Class<T> resultClass,
      Pair<KlabAsset.KnowledgeClass, Function<KlabAsset, List<KlabAsset>>>... functors) {
    ...
    for (var res : resourceSet.getResults()) {
      ...
      if (assetMap.containsKey(res.getResourceUrn())) {
        ...
        for (var asset : assetMap.get(res.getResourceUrn())) {
          ...
          if ((res.getResourceVersion() == null && asset.getSecond() == null)
              || (res.getResourceVersion() != null
                  && asset.getSecond() != null
                  && asset.getSecond().compatible(res.getResourceVersion()))) {
      ...
      if (doc.getUrn().equals(res.getResourceUrn())
          && res.getKnowledgeClass() == KlabAsset.classify(doc)) {
    ...
    String namespace =
        switch (res.getKnowledgeClass()) {
          case NAMESPACE,
              ...
              res.getResourceUrn();
          case MODEL, DEFINITION, CONCEPT_STATEMENT ->
              Utils.Paths.getLeading(res.getResourceUrn(), '.');
          default -> null;
        };
    ...
    if (namespace != null && documentMap.containsKey(namespace)) {
      for (var doc : documentMap.get(namespace)) {
        if (doc.getVersion() == null && res.getResourceVersion() == null
            || (res.getResourceVersion() != null
                && doc.getVersion() != null
                && doc.getVersion().compatible(res.getResourceVersion()))) {
          return doc;
        }
      }
    }
  ...
  private boolean ingestDocument(
    ...
    if (documentMap.containsKey(resource.getResourceUrn())) {
      for (var doc : documentMap.get(resource.getResourceUrn())) {
        if (doc.getVersion() == null && resource.getResourceVersion() == null
            || (resource.getResourceVersion() != null
                && doc.getVersion() != null
                && doc.getVersion().compatible(resource.getResourceVersion()))) {
          return true;
        }
      }
    }
