...
public class Utils extends org.integratedmodelling.klab.api.utils.Utils {
  ...
  public static class Time {
     ...
     */
    public static String formatDurationHMS(Duration duration) {
      if (duration == null) {
        return "--h--m--s";
      }
    ...
    /**
     * Returns a DateTimeFormatter that is permissive to optional format tokens. The supported
     * pattern is: <@{@code yyyy-M{1,2}-D{1,2}(T| )(H{1,2}(:)m{1,2}(:)s{1,2}(.S{0,6}(+NN)(Z)}
     */
     ...
     */
    public static List<LocalDate[]> splitIntoIntervals(
        LocalDate startDate, LocalDate endDate, int... exceptedMonths) {
      ...
      if (startDate.isBefore(endDate)) {
        if (exceptedMonths == null || exceptedMonths.length == 0) {
          ...
          for (int i = 0; i < months; i++) {
            ...
            currentDate =
                LocalDate.of(
                    currentDate.getYear(), currentDate.getMonth(), currentDate.lengthOfMonth());
            if (!distinctMonths.contains(currentDate.getMonth().getValue())) {
              // advance in iteration
              continue;
            }
            // if we have an except month, and it is not consecutive to the previous except month,
