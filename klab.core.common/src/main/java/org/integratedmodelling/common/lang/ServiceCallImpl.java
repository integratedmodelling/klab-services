...
public class ServiceCallImpl extends KimStatementImpl implements ServiceCall {
  ...
  @Serial private static final long serialVersionUID = 8447771460330621498L;
  ...
  public ServiceCallImpl(String name, Object... parameters) {
    ...
    this.sourceCode = encode(KlabLanguage.KIM);
  }

  @SuppressWarnings("unchecked")
  public ServiceCallImpl(ServiceCall serviceCall, Object... parameters) {
    this.urn = serviceCall.getUrn();
    this.parameters.putAll(serviceCall.getParameters());
    if (parameters != null && parameters.length == 1 && parameters[0] instanceof Map) {
      this.parameters.putAll((Map<String, ?>) (parameters[0]));
    } else if (parameters != null) {
      for (int i = 0; i < parameters.length; i++) {
        String key = parameters[i].toString();
        Object val = parameters[++i];
  ...
  private String stringValue(Object val) {
    ...
    if (val instanceof Encodeable) {
      ...
      return ((NumericRangeImpl) val).getLowerBound()
          + " to "
          + ((NumericRangeImpl) val).getUpperBound();
    } else if (val instanceof Classification) {
      String ret = "";
      for (Pair<Concept, Classifier> o : ((Classification) val)) {
        ret +=
            (ret.isEmpty() ? "" : ", ")
                + o.getSecond().getSourceCode()
                + " : '"
                + o.getFirst()
                + "'";
      }
  ...
  /**
   * Create a function call from the passed parameters. All parameters after the name must be given
   * in pairs: (string, value)*
   *
   * @param name
   * @param parameters
  ...
  /**
   * Return a service call for the same prototype with the passed arguments added as unnamed
   * parameters, starting at the first unassigned.
   *
   * <p>TODO should validate the arguments w.r.t. the prototype.
   *
  ...
  /**
   * Return a service call for the same prototype with the passed arguments, parsing the argument
   * list as key, value pairs. If a single map is passed as argument, that becomes the parameter
   * map.
   *
   * <p>TODO should validate the arguments w.r.t. the prototype.
   *
