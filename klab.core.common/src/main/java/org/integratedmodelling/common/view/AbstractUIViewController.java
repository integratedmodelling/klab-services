...
/**
 * The default abstract ancestor for views builds upon the annotations found on the class and
 * collaborates with the routing process implemented in the {@link AbstractUIController}.
 */
public abstract class AbstractUIViewController<T extends View> implements ViewController<T> {
  ...
  public <S extends KlabService> S service(
      KlabService.ServiceCapabilities capabilities, Class<S> serviceClass) {
    if (capabilities == null) {
      return null;
    }
    return ((AbstractUIController) controller)
        .serviceById(capabilities.getServiceId(), serviceClass);
  }
  ...
  public <P, T1 extends PanelView<P>> T1 openPanel(Class<T1> panelType, P payload) {
    ...
    if (cls == null) {
      controller
          ...
          .getOwner()
          .warn(
              "cannot find panel class implementation for "
                  + panelType.getCanonicalName()
                  + ": ignoring panel open request");
    }
   ...
   * candidates but the passed class is suitable and concrete, return the argument, otherwise return
   * null.
   *
   * @param candidate
   * @return a registered panel view implementation or null
   */
  protected Class<? extends PanelView<?>> getPanelViewClass(
      Class<? extends PanelView<?>> candidate) {
    ...
    boolean ok =
        (candidate.isInterface()
            ...
            || (candidate.getModifiers() & Modifier.PUBLIC) == 0);
    return ok ? candidate : null;
  }

  @Override
  public void registerPanelView(Class<? extends PanelView<?>> panelViewClass) {
    if (panelViewClass.isInterface()
        || panelViewClass.isArray()
        || panelViewClass.isAnnotation()
        || (panelViewClass.getModifiers() & Modifier.PUBLIC) == 0) {
      throw new KlabIllegalArgumentException(
          "Class " + panelViewClass + " is unsuitable as a panel " + "view implementation");
    }
  ...
  /**
   * Get the view. NOTE: may be null depending on when {@link #registerView(View)} gets called. If
   * so, messages should be stored and replayed when the view is available.
   *
