...
//  public void status(Scope.Status status) {}
  ...
  private void handleMatch(
      Pair<Message.MessageClass, Message.MessageType> key, EventMatcher matcher, Message message) {
    ...
    for (var arg : matchArguments) {
      if (arg instanceof Predicate<?>) {
        ...
        if (matcher.payloadPrototype != null) {
          error(
              "Internal: installing multiple payload prototypes for matching, overriding each other ("
                  + arg
                  + ")");
        }
