 ...
 * definition. A query for any instance stored by derived kboxes with its ID in
 * the set will return compatible observables.
 * </p>
...
//			}
	...
	/**
	 * Get the ID of the table that contains the "primary" object we provide. Used
	 * to check for empty database - if this is not there, either nothing needs to
	 * be done or initialization needs to be performed.
	 * 
	 * @return
	 */
	...
	/**
	 * Delete all objects in the passed namespace and return the number of objects
	 * deleted.
	 * 
	 * @param namespaceId
	 * @return
...
//					+ "isscenario BOOLEAN" + "); " + "CREATE INDEX namespace_id_index ON namespaces(id); ";
	...
	/**
	 * Get the ID correspondent to the passed concept, and if unavailable return -1.
	 * Does not use the database so it's very fast.
	 * 
	 * @param c
	...
	/**
	 * Check that the passed observable has been inserted, and if not make sure it
	 * is represented in the database. Return the stable ID to use for storing
	 * records that use it.
	 * 
	 * @param observable
	 * @param monitor
	...
	/**
	 * Determine all the compatible MODEL concepts for which observables have been
	 * stored, and return the set of their IDs.
	 * 
	 * If the core type is concrete, only that core type is looked up in the
	 * observable's parents, so that models that observe that type (potentially with
	 * other traits not adopted by the observable and in any compatible context) are
	 * found. If the core type is abstract or was stated generic, any child is OK as
	 * long as trait, roles, inherency and context are compatible.
	 * 
	 * @param observable
	...
	public Set<Long> getCompatibleTypeIds(Observable observable, Concept context) {
		...
		/*
		 * We lookup all models whose observable incarnates the core type, adding all
		 * possible specific models if the observable is abstract or the context
		...
		for (Concept candidate : getCandidates(main, observable.getDescriptionType().isInstantiation(),
			...
			/*
			 * let an abstract model resolve a concrete observable if the abstract traits
			 * are in the resolved predicates for the observable. If the observable contains
	...
	private Set<Concept> getCandidates(Concept concept, boolean instantiation,
			Map<Concept, Concept> resolvedPredicates) {
	...
	/**
	 * We only accept abstract parent concepts (up to the base observable) if the
	...
	protected void storeMetadataFor(long oid, Map<String, String> metadata) {
		...
		for (String s : metadata.keySet()) {
			...
			try {
				/*
				 * OK, must execute these right away unfortunately - so if something goes wrong
				 * with the object's storage these will remain in the DB.
				 */
	 ...
	 * namespace record - this should be done when this has returned true and there
	 * were no errors.
	 * 
