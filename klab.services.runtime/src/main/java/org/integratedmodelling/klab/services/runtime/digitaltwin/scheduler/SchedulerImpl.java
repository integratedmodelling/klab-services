...
public class SchedulerImpl implements Scheduler {
  ...
  enum Synchronicity {
    /**
     * Fully asynchronous tasks, started on time and left alone to complete in their own time while
    ...
    /**
     * Tasks are started without waiting for others to finish, but no task with start = t will be
     * started until all tasks with end <= t have finished.
     */
    TIME_SYNCHRONOUS
  }
  ...
  class Registration {
    ...
    /*
     * this is set by tagExpirations() just before running each set of registrations
     * in a slot. True means that this is the last active (rounds == 0) registration
     * for its specified time in the slot.
     */
    ...
    ////         * @param endtime
    ////         */
    ////        public Registration(Observation observation, IBehavior.Action scheduled, IScale
    // scale, IRuntimeScope scope,
    ////                            long endtime) {
    ////
    ////            this.scale = scale;
    ...
    ////                     */
    ////                    if (recipient instanceof IDirectObservation && !((IDirectObservation)
    // recipient).isActive()) {
    ////                        return ret;
    ////                    }
    ////
    ////                    /*
    ////                     * 2. Set the context at() the current time. This will also need to
    // expose any
    ////                     * affected outputs that move at a different (context) speed through a
    // rescaling
    ////                     * wrapper. Done within the context, which uses its current target to
    // establish
    ...
    ////                    ActorRef<KlabMessage> sender = ((ActorReference) ((Observation)
    // observation.getScope()
    ////
    ...
    ////                    recipient.getActor().tell(new KActorsMessage(sender, scheduled.getId(),
    // null, null,
    ////                                                                 new ActorScope(observation,
    // appId, transitionContext, null), appId));
    ////
    ...
    ////                                                        observation instanceof
    // IObservationGroup ? Mode.INSTANTIATION : Mode.RESOLUTION));
    ////
    ////                            IObservation parent = scope.getParentArtifactOf(observation);
    ////                            if (parent != null &&
    // scope.getWatchedObservationIds().contains(parent.getId())) {
    ////
    ...
    ////         * @param changes
    ////         */
    ////        public Registration(IObservation replayedObservation, IRuntimeScope scope, ITime[]
    // changes) {
    ////
    ...
    ////                public Collection<ObservedConcept> apply(IMonitor monitor) {
    ////
    ////                    IRuntimeScope runtimeScope = (IRuntimeScope)
    // replayedObservation.getScope();
    ...
    ////
    ////                        if
    // (runtimeScope.getImplicitlyChangingObservables().contains(tracked)) {
    ////                            if (monitor.isInterrupted()) {
    ////                                return ret;
    ////                            }
    ////                            Actuator actuator = getActuator(tracked, dependencyGraph);
    ////                            if (actuator != null) {
    ////                                reinitializeObservation(tracked, catalog.get(tracked),
    // actuator, time, runtimeScope,
    ...
    //
    //                    /*
    //                     * 2. Set the context at() the current time. This will also need to expose
    // any
    //                     * affected outputs that move at a different (context) speed through a
    // rescaling
    //                     * wrapper. Done within the context, which uses its current target to
    ...
    //
    //                    /*
    //                     * TODO if the target is a group of events, it has been filtered to only
    // contain
    ...
    //                                                          : (" at [" + new
    // Date(time.getStart().getMilliseconds()) + " - "
    //                                                                  + new
    // Date(time.getEnd().getMilliseconds()) + "]")));
    //
    ...
    //
    //                        /*
    //                         * 3. Run all contextualizers in the context that react to
    // transitions; check
    //                         * for signs of life at each step. Anything enqueued here is active so
    // no
    ...
    //                                transitionContext.getVariables().put(computation.targetId,
    ...
    //                            }
    //
    //                            IRuntimeScope transitionScope = actuator.setupScope(artifact,
    // transitionContext,
    //                                                                                statistics);
    //
    ...
    //                                }
    //
    // computation.contextualizer.notifyContextualizedResource(resource, target,
    //
    // transitionScope);
    //                            }
    //
    //                            if (resource.isFinal()) {
    //                                scope.getProvenance().add(artifact, resource, (IScale)
    // transitionScale, actuator,
    //                                                          transitionScope,
    // IAssociation.Type.wasDerivedFrom);
    //                            }
    //
    //                            /*
    //                             * substitute the target for the next computation if we're using
    // layers.
    ...
    //                                                                  resource, artifact,
    // transitionScope, (IScale) transitionScale, changedArtifacts,
    //                                                                  statistics);
    //
    //                            if (artifact == null) {
    //                                interruptedTargets.add(computation.targetId);
    ...
    //                                    // state layers and rescalers have the same observable so
    // this works
    //                                    ret.add(new ObservedConcept(((IObservation)
    // modified).getObservable()));
    //                                }
    //                            }
    //
    //                        }
    ...
    //                     */
    //                    if (target instanceof IDirectObservation && !((IDirectObservation)
    // target).isActive()) {
    //
    //                        ObservationChange change = new ObservationChange();
    //                        change.setContextId(scope.getRootSubject().getId());
    ...
    //
    //                            ((Observation) observation).finalizeTransition((IScale)
    // transitionScale);
    //
    //                            IObservation parent = scope.getParentArtifactOf(observation);
    //                            if (parent != null &&
    ...
    //
    // IMessage.MessageClass.ObservationLifecycle,
    //                                                            IMessage.Type.ModifiedObservation,
    // change));
    //                            } else {
    //                                ((Observation) observation).setDynamic(true);
    //                            }
  ...
  //                throw new KlabUnimplementedException("time-synchronous scheduling not
  // implemented");
  //            }
  //
  //            return null;
  ...
  //
  //    public static final long DEFAULT_RESOLUTION = TimeUnit.NANOSECONDS.convert(10,
  ...
  //    // views can do this.
  //    private Map<Pair<IDirectObservation, Dataflow>, List<Registration>> terminationRegistrations
  // = new HashMap<>();
  //    private IRuntimeScope runtimeScope;
  //    private Map<IObservedConcept, IObservation> catalog; // only for the registrations that
  // replay
  //    // change
  //
  //    Map<Pair<IDirectObservation, Dataflow>, List<Registration>> dataflows;
  //
  //    public Scheduler(IRuntimeScope runtimeScope, String contextId, IResolutionScope scope,
  // IMonitor monitor) {
  ...
  //    }
  //
  //    public void schedule(Action action, Observation targetObservation, Time time, RuntimeScope
  // runtimeScope) {
  //
  //        /*
  ...
  //
  //        if (start == null || (overall.getTime().getStart() != null &&
  // start.isBefore(overall.getTime().getStart()))) {
  //            start = overall.getTime().getStart();
  //        }
  //        if (end == null || (overall.getTime().getEnd() != null &&
  // start.isAfter(overall.getTime().getEnd()))) {
  ...
  //        }
  //
  //        final long endTime = overall.getTime().getEnd() == null ? -1 :
  // overall.getTime().getEnd().getMilliseconds();
  //
  //        registrations.add(new Registration(targetObservation, action, scale, runtimeScope,
  // endTime));
  //    }
  //
  ...
  // (targetObservation.getObservable().is(IKimConcept.Type.PROCESS)
  ...
  //        }
  //
  //        registrations.add(new Registration(actuator, computations, (IDirectObservation)
  // targetObservation, scale, scope,
  ...
  //
  //        /*
  //         * 1. Collect all dataflows from actuators and pair them to their actuators.
  ...
  //
  //        /*
  //         * 2. Schedule each dataflow independently to align internal dependencies and
  //         * change detection.
  //         */
  ...
  //    private void schedule(Pair<IDirectObservation, Dataflow> dataflow, List<Registration>
  // actions,
  ...
  //    }
  //
  //    private List<Registration> separateTerminationRegistrations(Pair<IDirectObservation,
  // Dataflow> dataflow,
  //                                                                List<Registration>
  // registrations, Set<IObservedConcept> relevant) {
  //
  ...
  // ObservedConcept(registration.replayedObservation.getObservable())))
  ...
  //                                                             Graph<IObservedConcept,
  // DefaultEdge> dependencies) {
  //
  //        /*
  ...
  //         */
  //        Graph<IObservedConcept, DefaultEdge> dynamicDependencies = new
  // DefaultDirectedGraph<>(DefaultEdge.class);
  //        for (Registration r : registrations) {
  //            if (r.actuator != null) {
  //                dynamicDependencies.addVertex(new ObservedConcept(r.actuator.getObservable(),
  // r.actuator.getMode()));
  //            } else if (r.replayedObservation != null) {
  //                dynamicDependencies
  //                        .addVertex(new ObservedConcept(r.replayedObservation.getObservable(),
  // Mode.RESOLUTION));
  //            }
  //        }
  //
  ...
  //                    }
  //
  //                    if (dependencies.containsVertex(changing) && Graphs.dependsOn(dc, changing,
  // dependencies)) {
  //
  //                        /*
  //                         * try adding the dependency on a clone. If that creates cycles, warn
  // and move
  //                         * on.
  //                         */
  //                        Graph<IObservedConcept, DefaultEdge> clone =
  // Graphs.copy(dynamicDependencies,
  //                                                                                 new
  // DefaultDirectedGraph<>(DefaultEdge.class));
  //                        clone.addEdge(oc, dc);
  //                        CycleDetector<IObservedConcept, DefaultEdge> cycles = new
  // CycleDetector<>(clone);
  ...
  //                                                 + " ignored because of circular dependencies:
  // consider review of model logics");
  //                            continue;
  //                        }
  ...
  //        for (Registration r : registrations) {
  //            if (r.actuator != null) {
  //                ObservedConcept oob = new ObservedConcept(r.actuator.getObservable(),
  // r.actuator.getMode());
  ...
  //                rgs.add(r);
  //            } else if (r.replayedObservation != null) {
  //                ObservedConcept oob = new ObservedConcept(r.replayedObservation.getObservable(),
  // Mode.RESOLUTION);
  //                List<Registration> rgs = index.get(oob);
  //                if (rgs == null) {
  //                    rgs = new ArrayList<>();
  ...
  //
  //        List<Registration> ret = new ArrayList<>();
  //        TopologicalOrderIterator<IObservedConcept, DefaultEdge> order = new
  // TopologicalOrderIterator<IObservedConcept, DefaultEdge>(
  //                dynamicDependencies);
  //
  //        while (order.hasNext()) {
  ...
  //
  //            if (monitor.isInterrupted() || monitor.getWaitTime() >
  // Configuration.INSTANCE.getMaxWaitTime()) {
  ...
  //                            // FIXME NO! Must run in batches all those starting at the same
  // time,
  //                            // then wait
  ...
  //
  //                        Map<IObservedConcept, IObservation> catalog =
  // registration.scope.getCatalog();
  //
  //                        reschedule(registration, false);
  //
  //                        /*
  //                         * check for implicitly affected actuators. This must be done when the
  // last
  //                         * registration with each particular delay has finished.
  //                         */
  //                        if (toRun != null && registration.endsPeriod && changed.size() > 0) {
  //
  //                            // must be here as each registration may be for a different object
  // with
  //                            // a
  //                            // different scope.
  //                            DependencyGraph dependencyGraph =
  // registration.scope.getDependencyGraph();
  //
  //                            for (IObservedConcept tracked : dependencyGraph) {
  //
  ...
  //
  //                                if
  // (registration.scope.getImplicitlyChangingObservables().contains(tracked)
  ...
  //                                    reinitializeObservation(tracked, catalog.get(tracked),
  //                                                            getActuator(tracked,
  // dependencyGraph), toRun, runtimeScope,
  //
  // registration.actuator.getDataflow());
  //
  //                                    changed.add(tracked);
  ...
  //                monitor.send(Message.create(session.getId(),
  // IMessage.MessageClass.ObservationLifecycle,
  ...
  //                                                    ((Time)
  // changing.getScale().getTime()).getChangedExtents()));
  //        }
  //    }
  //
  ...
  //     * @param runtimeScope
  //     */
  //    private void reinitializeObservation(IObservedConcept observable, IObservation target,
  // Actuator actuator,
  //                                         ITime time, IRuntimeScope runtimeScope, IDataflow<?>
  // dataflow) {
  //
  //        if (target != null) {
  //
  //            ILocator transitionScale = runtimeScope.getResolutionScale().at(time);
  //            IRuntimeScope transitionContext =
  // runtimeScope.targetToObservation(target).locate(transitionScale, monitor);
  //            Set<IArtifact> changedArtifacts = new HashSet<>();
  //
  //            /*
  ...
  // runtimeScope.getStatistics().forTarget(actuator).schedulerStep();
  //
  //            try {
  //
  ...
  //                if (parent != null &&
  // runtimeScope.getWatchedObservationIds().contains(parent.getId())) {
  //
  //                    ObservationChange change = new ObservationChange();
  ...
  //                        change.setType(ObservationChange.Type.ValueChange);
  //                    } else if (target instanceof IDirectObservation && !((IDirectObservation)
  // target).isActive()) {
  //                        change.setType(ObservationChange.Type.Termination);
  //                    }
  //
  ...
  //     */
  //    private Actuator getActuator(IObservedConcept observable, Graph<IObservedConcept,
  // DefaultEdge> dependencies) {
  ...
  //     * it to run dependencies that were not scheduled after it.
  //     *
  //     * @param registration
  //     * @param first
  //     * @return
  ...
  //         */
  //        if (registration.timeChanges != null && registration.changeIndex <
  // registration.timeChanges.length - 1) {
