...
public abstract class KnowledgeGraphNeo4j extends AbstractKnowledgeGraph {
    ...
    /**
     * Predefined Cypher queries. FIXME some should be substituted by programmatic queries, leaving only graph initialization
     */
    interface Queries {
    ...
    private String getLabel(Object target) {
        ...
        if (target instanceof KnowledgeGraphQuery.AssetType assetType) {
            return switch (assetType) {
                ...
                case DATA -> "Data";
                default -> throw new KlabInternalErrorException("Cannot find a KG node label for " + assetType);
            };
        }
        ...
        if (ret == null && target instanceof RuntimeAsset runtimeAsset) {
            ret =
                    switch (runtimeAsset.classify()) {
                        ...
                        case DATAFLOW -> "Dataflow";
                        case PROVENANCE -> "Provenance";
                        default -> throw new KlabIllegalStateException("Unexpected value: " + runtimeAsset.classify());
                    };
        }
