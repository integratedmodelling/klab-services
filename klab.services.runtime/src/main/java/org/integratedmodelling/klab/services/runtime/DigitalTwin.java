...
/**
 * The actual observation content kept in each local context (inside the context
 * agent or the scope itself if local). Contains the influence diagram between
 * observations with the log of any modification event timestamp, the scheduler,
 * and the catalog of ID->{observation, actuator}. Also maintains the logical
 * and physical "family trees" of observation. The logical one skips the
 * instance container built for the instantiators, which is kept in the physical
 * structure.
 * <p>
 * Essentially the implementation of the "digital twin" accessed through the
 * {@link ContextScope}. Eventually it can have its own API although the
 * {@link ContextScope} should have all it needs to interact with it.
 * 
 * @author Ferd
 *
 */
public class DigitalTwin {
	...
	/**
	 * Key to locate this in the context scope data. The first contextualization
	 * creates it and it remains the same object throughout the context lifetime.
	 */
	...
	class InfluenceEdge extends DefaultEdge {
		...
		/*
		 * Scheduler timestamps corresponding to this link having caused modifications
	...
	/**
	 * Events that must reach the client side are communicated through this.
	 */
	Channel bus;
	...
	/**
	 * The influence diagram tells us which observation is influenced by changes in
	 * which others (info mutuated by actuators at first, then potentially modified
	 * through behaviors or messages). Holds for actuators and observations because
	 * the IDs are the same. The edges should keep the list of modification
	 * timesteps.
	 */
	Graph<String, DefaultEdge> influenceDiagram = new DefaultDirectedGraph<>(DefaultEdge.class);

	/**
	 * TODO if Javolution or something else eventually provides a fast map, use that
	 * here.
	 */
	...
	/*
	 * TODO a separate Inspector for debugging and testing (ideally, use a different
	 * top contextualizer that talks to the inspector after checking if the
	 * inspector is not null, instead of checking at every step).
	 */

	/**
	 * Run the passed actuator, building the necessary observations and updating all
	 * records. Sound dependency order must be guaranteed by the caller. Notify
	 * whatever info we have subscribed to.
	 * 
	 * @param actuator
	 * @param scope
	 * @return
	...
	/**
	 * Call at scope disposal and service shutdown (scope disposal should be
	 * scheduled after configured inactivity).
	 * 
	 * @return
	 */
	...
	public void link(Observation childArtifact, Observation parentArtifact) {
		...
		if (parentArtifact instanceof Process) {
			...
			/*
			 * we keep the information about the artifact being owned by a process, so that
			 * we can tell the occurrence when it's relevant.
			 */
