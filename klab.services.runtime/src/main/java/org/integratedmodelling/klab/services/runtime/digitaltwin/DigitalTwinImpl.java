...
public class DigitalTwinImpl implements DigitalTwin {
       ...
       */
            try (var kgTransaction = knowledgeGraph.createTransaction()) {
                ...
                for (var edge : graph.edgeSet()) {
                    ...
                    this.graphReference.getEdges().add(encodeLink(source, target, edge.relationship, relationshipData));
                ...
                if (primary && !trivial) {
                    ...
                    this.graphReference.getEdges().add(encodeLink(knowledgeGraph.provenance(), activity, GraphModel.Relationship.HAS_CHILD));
                }
    ...
    private RuntimeAssetGraph.Edge encodeLink(RuntimeAsset source, RuntimeAsset target, GraphModel.Relationship relationship, Object... relationshipData) {
    ...
    private RuntimeAssetGraph.Node encodeRuntimeAsset(RuntimeAsset asset, Map<String, RuntimeAssetGraph.Node> nodes) {
        if (asset == knowledgeGraph.scope()) {
        ...
        return nodes.computeIfAbsent(asset.getId() + "", id -> {
