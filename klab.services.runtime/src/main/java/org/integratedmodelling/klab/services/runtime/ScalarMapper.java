...
/**
 * Scalar executor providing handling and caching for LUTs, classifications and expression and implementing
 * the mapping strategy configured in the runtime.
 */
public class ScalarMapper {
    ...
    public ScalarMapper(Observation target, DigitalTwin digitalTwin, ServiceContextScope scope) {
        ...
        this.storageClass = switch (target.getObservable().getArtifactType()) {
            ...
            default ->
                    throw new KlabIllegalStateException("scalar mapping to type " + target.getObservable().getArtifactType() + " not supported");
        };
    }
    ...
    public boolean run() {
        ...
        // TODO masking
        switch (storage) {
            case DoubleStorage doubleStorage -> { doubleStorage.map(getDoubleMapper());}
            case FloatStorage doubleStorage -> {}
            case BooleanStorage booleanStorage -> {}
            case KeyedStorage keyedStorage -> {}
            default -> throw new KlabInternalErrorException("unexpected storage type in ScalarMapper run()");
        }
