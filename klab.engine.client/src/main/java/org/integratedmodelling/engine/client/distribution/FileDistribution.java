...
/**
 * A proxy to efficiently synchronize any web-hosted software distribution based on a single product
 * directory, with intelligent synchronization from an appropriately configured remote host. The remote host
 * must contain a file list with file hashes named <code>filelist.txt</code> generated at the server side by
 * issuing
 *
 * <pre>
 ...
 * </pre>
 * <p>
 * in the root directory of the distribution, or from the Java API, ing
 * {org.integratedmodelling.common.utils.Utils.Files#createMD5Digest(File, String)}.
 *
 * @author Ferd
 */
public class FileDistribution {
    ...
    public static interface SyncListener {
        ...
        /**
         * This is only called when preparing an incremental update from a previous distribution, which can
         * run relatively long.
         */
     ...
     * @param workspace
     */
    public FileDistribution(String url, File workspace) {
        remoteURL = url;
    ...
    /**
     * Weak check for an existing distribution. Should actually check for all files in the list, but who wants
     * to do that. When fixed, the filelist should be the last file downloaded, so that's a relatively
     * meaningful check.
     *
     * @return true if the last file in the list has been downloaded successfully.
     */
    public boolean isComplete() {
        return workspace != null && workspace.exists() && new File(workspace + File.separator + "filelist" +
                ".txt").exists();
    }

    /**
     * Synchronize the necessary files. Will do nothing (and return true) if we have elected to use a local
     * installation. Will return false if we're not network-enabled or the selected server is offline.
     *
    ...
    private void scanForDeletion(File file, HashMap<String, String> remote, ArrayList<File> toRemove) {
        ...
        if (file.isDirectory()) {
            ...
            if (!fname.isEmpty() && !fname.equals("filelist.txt") && !fname.endsWith(".log") && !remote.containsKey(fname)) {
                toRemove.add(file);
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        FileDistribution tl = new FileDistribution("http://www.integratedmodelling" + ".org/downloads" +
                "/products/master/cli/102", new File(System.getProperty("user.home") + File.separator +
                "scratchmyass"));
        tl.setListener(new SyncListener() {
            ...
            public void beforeDownload(String file) {
                tl.scope.debug("downloading " + file + " (" + NumberFormat.getPercentInstance().format((double) _sofar / (double) _total) + ")");
                _sofar++;
            }

            @Override
            public void beforeDelete(File localFile) {
                tl.scope.debug("deleting " + localFile);
            }

            @Override
            public void notifyDownloadCount(int downloadFilecount, int deleteFileCount) {
                tl.scope.debug(downloadFilecount + " to download, " + deleteFileCount + " to delete, ");
                _total = downloadFilecount;
            }

            @Override
            public void transferFinished(Exception e) {
                tl.scope.debug("transferred " + _sofar + " files");
                if (e != null) {
                    tl.scope.error("error: " + e);
                }
            }

            @Override
            public void notifyFileProgress(String file, long bytesSoFar, long totalBytes) {
                tl.scope.debug("   " + file + ": " + bytesSoFar + "/" + totalBytes);
            }
            ...
            public void notifyError(Exception e) {
                tl.scope.error(e.getLocalizedMessage());
            }
        });
